...
class Library_Data_Analysis
{
    ...
    public static function slabs($slabs)
    {
        ...
        foreach ($slabs as $id => $slab) {
            ...
            if (is_numeric($id)) {
                ...
                $mem_requested = isset($slab['mem_requested']) ? $slab['mem_requested'] : 0;
                ...
                $slabs[$id]['mem_wasted'] = (($slab['total_chunks'] * $slab['chunk_size']) < $mem_requested) ? (($slab['total_chunks'] - $slab['used_chunks']) * $slab['chunk_size']) : (($slab['total_chunks'] * $slab['chunk_size']) - $mem_requested);
    ...
    {
        if ($uptime > 0) {
            $days = intval(floor($uptime / 60 / 60 / 24));
            // Primero, calcula las horas totales como un entero
            $totalHours = intval($uptime / 60 / 60);
            // Luego aplica el operador módulo para obtener las horas restantes
            $hours = $totalHours % 24;

            // Similarmente, primero calcula los minutos totales como un entero
            $totalMinutes = intval($uptime / 60);
            // Luego aplica el operador módulo para obtener los minutos restantes
    ...
    public static function uptime_OLD($uptime)
    {
        if ($uptime > 0) {
