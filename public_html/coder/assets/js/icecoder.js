...
// ======
    ...
    changeFilesW: function (expandContract) {
        if (false === this.lockedNav || this.filesW === this.minFilesW) {
            if ("undefined" !== typeof this.changeFilesInt) {
                clearInterval(this.changeFilesInt)
            }
            this.changeFilesInt = setInterval(function (ic) {
                ic.changeFilesWStep(expandContract)
            }, 10, this);
        }
    },
...
// ======
    ...
    cMonChange: function (thisCM, cMinstance, changeObj, cM) {
        let sels, rData, theTag, thisLine, thisChar, tagInfo, charDiff, closeDiff, repl1, repl2, filepath, filename,
            fileExt;
            ...
            for (let i = 0; i < this.oppTagReplaceData.length; i++) {
                ...
                if ("undefined" !== typeof tagInfo) {
                    ...
                    if ("undefined" !== typeof changeObj.from) {
                        ...
                        repl2 = {
                            line: thisLine,
                            ch: thisChar + charDiff + ("open" === tagInfo.at ? 2 : closeDiff) + rData[0].length
                        };
                    }
                }
            ...
            for (let i = 0; i < opcodes.length; i++) {
                ...
                if ("equal" !== opcodes[i][0]) {
                    ...
                    if ("replace" === opcodes[i][0]) {
                        ...
                        for (let j = 0; j < (opcodes[i][2]) - (opcodes[i][1]); j++) {
                            ...
                            cM.markText({
                                ...
                                line: opcodes[i][3] + j + this.renderPaneShiftAmount,
                                ch: sDiffs[0]
                            }, {className: "diffGreyLighter"});
                            cM.markText({
                                line: opcodes[i][1] + j,
                                ch: sDiffs[0]
                            }, {
                                line: opcodes[i][3] + j + this.renderPaneShiftAmount,
                                ch: sDiffs[0] + sDiffs[1]
                            }, {className: "diffGrey"});
                            cM.markText({
                                line: opcodes[i][1] + j,
                                ch: sDiffs[0] + sDiffs[1]
                        ...
                        cM.markText({line: opcodes[i][1], ch: 0}, {
                            line: opcodes[i][2] - 1,
                            ch: 1000000
                        }, {className: "diffGreen"});
                    }
                    ...
                    if ("replace" !== opcodes[i][0] && opcodes[i][1] === opcodes[i][2]) {
                        ...
                        cM.markText({line: opcodes[i][2] - 1, ch: 0}, {
                            line: opcodes[i][2] - 1,
                            ch: 1000000
                        }, {className: "diffNone"});
                    }
                    ...
                    if ("replace" === opcodes[i][0]) {
                        ...
                        for (let j = 0; j < (opcodes[i][4]) - (opcodes[i][3]); j++) {
                            sDiffs = (this.findStringDiffs(cM.getLine(opcodes[i][1] + j), cMdiff.getLine(opcodes[i][3] + j)));
                            cMdiff.markText({
                                line: opcodes[i][1] + j - this.renderPaneShiftAmount,
                                ch: 0
                            }, {line: opcodes[i][3] + j, ch: sDiffs[0]}, {className: "diffGreyLighter"});
                            cMdiff.markText({
                                line: opcodes[i][1] + j - this.renderPaneShiftAmount,
                                ch: sDiffs[0]
                            }, {line: opcodes[i][3] + j, ch: sDiffs[0] + sDiffs[2]}, {className: "diffGrey"});
                            cMdiff.markText({
                                line: opcodes[i][1] + j - this.renderPaneShiftAmount,
                                ch: sDiffs[0] + sDiffs[2]
                            }, {line: opcodes[i][3] + j, ch: 1000000}, {className: "diffGreyLighter"});
                        }
                        // Deleting
                    } else {
                        cMdiff.markText({line: opcodes[i][3], ch: 0}, {
                            line: opcodes[i][4] - 1,
                            ch: 1000000
                        }, {className: "diffRed"});
                    }
                    ...
                    if ("replace" !== opcodes[i][0] && opcodes[i][3] === opcodes[i][4]) {
                        ...
                        // Mark the range with empty class
                        cMdiff.markText({line: opcodes[i][4] - 1, ch: 0}, {
                            line: opcodes[i][4] - 1,
                            ch: 1000000
                        }, {className: "diffNone"});
                    }
    ...
    moveLines: function (dir) {
        ...
        if ("up" === dir && 0 < lineStart.line) {
            swapLineNo = lineStart.line - 1
        }
        if ("down" === dir && lineEnd.line < thisCM.lineCount() - 1) {
            ...
            thisCM.operation(function () {
                ...
                if ("up" === dir) {
                    for (let i = lineStart.line; i <= lineEnd.line; i++) {
                        thisCM.replaceRange(thisCM.getLine(i), {line: i - 1, ch: 0}, {
                            line: i - 1,
                            ch: 1000000
                        }, "+input");
                    }
                ...
                thisCM.replaceRange(swapLine, {
                    line: "up" === dir ? lineEnd.line : lineStart.line,
                    ch: 0
                }, {line: "up" === dir ? lineEnd.line : lineStart.line, ch: 1000000}, "+input");
    ...
    tagWrapper: function (tag) {
        ...
        if ('div' === tag) {
            ...
            if (
                ...
                if ("a" === tag) {
                    thisCM.setCursor({line: thisCM.getCursor('start').line, ch: thisCM.getCursor('start').ch + 9})
                }
            }
        }
    },
    ...
    duplicateLines: function (line) {
        ...
        if (!line && thisCM.somethingSelected()) {
            ...
            thisCM.replaceRange(thisCM.getLine(line) + "\n" + thisCM.getLine(line), {line: line, ch: 0}, {
                line: line,
                ch: 1000000
            }, "+input");
                ...
                this.goFindAfterOpenInt = setInterval(function (result) {
                    if (ICEcoder.openFiles[ICEcoder.selectedTab - 1] == result.filePath.replace(docRoot, "") && !ICEcoder.loadingFile) {
                        ...
                        setTimeout(function (result) {
                            ...
                            thisCM.setSelection({
                                line: result.range.from.line,
                                ch: result.range.from.ch
                            }, {line: result.range.to.line, ch: result.range.to.ch});
                        }, 20, result);
    ...
    updateFunctionClassList: function () {
        ...
        if (cM) {
            ...
            cM.doc.eachLine(function (handle) {
                ICEcoder.updateFunctionClassListItems(handle)
            });
        }
    },
    ...
    caretLocationType: function () {
        ...
        if (caretChunk.lastIndexOf("<\?") > caretChunk.lastIndexOf("?\>") && "Unknown" === caretLocType) {
            caretLocType = "PHP";
        } else if (caretChunk.lastIndexOf("<\%") > caretChunk.lastIndexOf("%\>") && "Unknown" === caretLocType) {
            caretLocType = "Ruby";
        } else if (caretChunk.lastIndexOf("<script\>") > caretChunk.lastIndexOf("<\/script>") && "Unknown" === caretLocType) {
            caretLocType = "JavaScript";
        } else if (caretChunk.lastIndexOf("<style") > caretChunk.lastIndexOf("/style>") && "Unknown" === caretLocType) {
            caretLocType = "CSS";
        } else if (caretChunk.lastIndexOf("<") > caretChunk.lastIndexOf(">") && "Unknown" === caretLocType) {
            caretLocType = "HTML";
        } else if ("Unknown" === caretLocType) {
            caretLocType = "Content";
        }
    ...
    lineCommentToggleSub: function (cM, cursorPos, linePos, lineContent, lCLen) {
            ...
            if (cM.somethingSelected()) {
                ...
                if (-1 < ["Ruby", "Python", "Erlang", "Julia", "YAML", "Perl"].indexOf(this.caretLocType)) {
                    ...
                    for (let i = startLine; i <= endLine; i++) {
                        cM.replaceRange(cM.getLine(i).slice(0, commentCH.length) != commentCH
                            ...
                            : cM.getLine(i).slice(commentCH.length, cM.getLine(i).length), {line: i, ch: 0}, {
                            line: i,
                            ch: 1000000
                        }, "+input");
                    }
                ...
                if (-1 < ["CSS", "SQL"].indexOf(this.caretLocType)) {
                    cM.replaceRange(lineContent.slice(0, commentBS.length) != commentBS
                        ...
                        : lineContent.slice(commentBS.length, lCLen - commentBE.length), {
                        line: linePos,
                        ch: 0
                    }, {line: linePos, ch: 1000000}, "+input");
                    adjustCursor = commentBS.length;
                    if (lineContent.slice(0, commentBS.length) == commentBS) {
                        adjustCursor = -adjustCursor
                    }
                } else {
                    cM.replaceRange(lineContent.slice(0, commentCH.length) != commentCH
                        ? commentCH + lineContent
                        : lineContent.slice(commentCH.length, lCLen), {line: linePos, ch: 0}, {
                        line: linePos,
                        ch: 1000000
                    }, "+input");
                    adjustCursor = commentCH.length;
                    if (lineContent.slice(0, commentCH.length) == commentCH) {
                        adjustCursor = -adjustCursor
                    }
                }
            }
        ...
        if (!cM.somethingSelected()) {
            cM.setCursor(linePos, cursorPos + adjustCursor)
        }
    },
...
// =====
            ...
            if (ctrlSim || evt.ctrlKey || this.cmdKey) {
                ...
                if (0 < this.selectedFiles.length && startFile.substr(0, startFile.lastIndexOf("|")) === endFile.substr(0, endFile.lastIndexOf("|"))) {
                    for (let i = 0; i < 1000000; i += 2) {
                        ...
                        if ("undefined" === typeof dirList.childNodes[i] || dirList.childNodes[i].nodeName !== "LI") {
                            break;
                        }
            ...
            try {
                ...
                for (let i = 0; i < opcodes.length; i++) {
                    ...
                    if ("equal" !== opcodes[i][0]) {
                        ...
                        this.getThisCM().replaceRange(newContent, {
                            line: opcodes[i][1] - docShift - startShift,
                            ch: 0
                        }, {line: opcodes[i][2] - docShift - endShift, ch: 1000000}, "+input");
    ...
    updateFileManagerList: function (action, location, file, perms, oldName, uploaded, fileOrFolder) {
        let actionElemType, cssStyle, targetElem, locNest, newText, innerLI, permColors, newUL, newLI, elemType, nameLI,
            shortURL;
        ...
        if ("add" === action && !get('filesFrame').contentWindow.document.getElementById(location.replace(iceRoot, "").replace(/\/$/, "").replace(/\//g, "|") + "|" + file)) {
            ...
            if (!locNest || 3 > locNest.childNodes.length) {
                ...
                newLI.ondragstart = function (event) {
                    parent.ICEcoder.addDefaultDragData(this, event)
                };
                newLI.ondrag = function (event) {
                    parent.ICEcoder.draggingWithKeyTest(event);
                    if (parent.ICEcoder.getcMInstance()) {
                        parent.ICEcoder.editorFocusInstance.indexOf('diff') == -1 ? parent.ICEcoder.getcMInstance().focus() : parent.ICEcoder.getcMdiffInstance().focus()
                    }
                };
                newLI.ondragover = function (event) {
                    parent.ICEcoder.setDragCursor(event, "folder" === actionElemType ? 'folder' : 'file')
                };
                newLI.ondragend = function () {
                    parent.ICEcoder.dropFile(this)
                };
            ...
            if (this.results.length > 0) {
                ...
                if (false === selectNext) {
                    ...
                    thisCM.setSelection(
                        ...
                        {
                            "line": this.results[this.findResult][0] - 1,
                            "ch": this.results[this.findResult][1] + rExpMatch0String.length
                        }
                    );
                    this.focus();
                }
...
// ==============
    ...
    dragCursorTest: function () {
        ...
        if (this.ready) {
            ...
            if (this.content.contentWindow.document && this.filesFrame.contentWindow) {
                ...
                if (zone = this.content.contentWindow.document.body) {
                    zone.style.cursor = cursorName
                }
                if (zone = this.filesFrame.contentWindow.document.body) {
                    zone.style.cursor = cursorName
                }
            }
        }
        ...
        if (thisCM) {
            ...
            if (true === this.codeAssist && "CSS" === this.caretLocType) {
                ...
                newBlock.onclick = function () {
                    ICEcoder.showColorPicker(match[0])
                };
    ...
    insertColorValue: function (color) {
        ...
        thisCM.replaceRange(color, {line: thisCM.getCursor().line, ch: cursor.start}, {
            line: thisCM.getCursor().line,
            ch: cursor.end
        }, "+input");
    },
...
// ====
    ...
    sortTabs: function (newOrder) {
        let a, b, savedPoints = [], savedContents = [], openFiles = [], openFileMDTs = [], openFileVersions = [],
            cMInstances = [], selectedTabWillBe;
...
// ==
    ...
    doSnippet: function (tgtString, replaceString) {
        ...
        if (-1 < lineContents.indexOf(tgtString)) {
            ...
            thisCM.replaceRange(replacedLine.replace("CURSOR", ""), {line: lineNo, ch: 0}, {
                line: lineNo,
                ch: 1000000
            }, "+input");
