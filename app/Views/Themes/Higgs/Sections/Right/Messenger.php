...
<div class="messenger col-12 w-100 h-100 p-0 position-fixed">


    <div class="row m-0 p-0">
        <div id="messenger" class="col-12 h-100 position-relative" style="width: 14.5rem;">
            <ul id="messenger-users" class="nav nav-pills flex-column mb-auto m-0">
                <?php if (is_array($messenger_users)) { ?>
                    <?php foreach ($messenger_users as $user) { ?>
                        <?php if ($user['user'] != safe_get_user()) { ?>
                            <li id="user-<?php echo($user['user']); ?>"
                                class="nav-item user-item w-100 overflow-hidden border-bottom">
                                <div class="d-flex align-items-center position-relative mb-0  ">
                                    <div class="avatar avatar-2xl status-online">
                                        <img class="rounded-circle border" src="<?php echo($user['avatar']); ?>"
                                             alt="">
                                    </div>
                                    <div class="mx-1">
                                        <h6 class="mb-0 fw-semi-bold  truncate">
                                            <a class="text-900 stretched-link " href="#">
                                                <?php echo($s->get_Wrap(@$user['name'], 24)); ?>
                                            </a>
                                        </h6>
                                        <p class="text-500 fs--2 mb-0">
                                            @<?php echo($s->get_Strtolower($user['alias'])); ?></p>
                                    </div>
                                </div>
                            </li>
                        <?php } ?>
...
<script type="module">
    ...
    const q = query(onlineCollection, where("instance", "==", "<?php echo(safe_get_instance());?>"));
    ...
    setInterval(async function () {
        const uid = 'user-<?php echo(safe_get_user());?>';
        const docRef = doc(db, 'online', uid);
        try {
            await setDoc(docRef, {
                instance: '<?php echo(safe_get_instance());?>',
                user: '<?php echo(safe_get_user());?>',
                name: '<?php echo(safe_get_user_fullname());?>',
                alias: '<?php echo(safe_get_user_alias());?>',
                avatar: '<?php echo(safe_get_user_avatar());?>',
                timestamp: Date.now()
            }, {merge: true});
