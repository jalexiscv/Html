 ...
 */
class Plans_Whys extends Model
{
    protected $table = "plans_whys";
    protected $primaryKey = "why";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'why' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
     ...
     */
    public function get_List($cause, int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->where("cause", $cause)
            ->groupStart()
     ...
     */
    function get_Total($cause, string $search = ""): int
    {
        $result = $this
            ->where("cause", $cause)
            ->groupStart()
     ...
     * @return array|false
     */
    public function get_Why($why): false|array
    {
        $key = $this->get_CacheKey("why-{$why}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $why)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
