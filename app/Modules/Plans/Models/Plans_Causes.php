 ...
 */
class Plans_Causes extends Model
{
    protected $table = "plans_causes";
    protected $primaryKey = "cause";
...
//$forge->addKey('profile');
     ...
     */
    public function get_List($plan, int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->where("plan", $plan)
            ->groupStart()
     ...
     * @return array|false
     */
    function get_MajorCause($plan): array|false
    {
        $result = $this
            ->where("plan", $plan)
            ->orderBy("score", "DESC")
     ...
     * @return array|false
     */
    public function get_Cause($cause): false|array
    {
        $key = $this->get_CacheKey("cause-{$cause}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $cause)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
