 ...
 */
class Plans_Causes_Scores extends Model
{
    protected $table = "plans_causes_scores";
...
//$forge->addKey('profile');
     ...
     * @return array|false
     */
    public function get_Score($score): false|array
    {
        $key = $this->get_CacheKey("score-{$score}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $score)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
