 ...
 */
class Plans_Actions extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'variables' => ['type' => 'TEXT', 'null' => FALSE],
                'alternatives' => ['type' => 'TEXT', 'null' => FALSE],
                'implementation' => ['type' => 'TEXT', 'null' => FALSE],
                'evaluation' => ['type' => 'TEXT', 'null' => FALSE],
                'percentage' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
                'start' => ['type' => 'DATE', 'null' => FALSE],
                'end' => ['type' => 'DATE', 'null' => FALSE],
                'status' => ['type' => 'ENUM', 'constraint' => ['PROPOSED', 'APPROVED', 'REJECTED'], 'default' => 'PROPOSED'],
                'owner' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->like("action", "%{$search}%")
            ->orLike("plan", "%{$search}%")
            ->orLike("variables", "%{$search}%")
            ->orLike("alternatives", "%{$search}%")
            ->orLike("implementation", "%{$search}%")
    ...
    /**
     * Obtiene el listado de acciones asociadas aun plan especifico.
     * @param int $limit
     * @param int $offset
     * @param string $plan
     * @return array|false
     */
    public function get_ListByPlan(string $plan = ""): array|false
    {
        $action= $this->where("plan", $plan)->findAll();
        if (is_array($action)) {
            return $action;
        } else {
            return false;
     ...
     * @return array|false
     */
    public function get_Action($action): false|array
    {
        $key = $this->get_CacheKey("action-{$action}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $action)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
