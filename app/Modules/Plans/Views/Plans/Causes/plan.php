...
foreach ($causes as $cause) {
    ...
    if (($mayor["score"] > 0) && ($mayor["cause"] === $cause["cause"])) {
        $code .= "    <a class=\"btn btn-sm btn-danger {$status}\" href=\"/plans/whys/list/{$cause["cause"]}?c={$mayor["score"]}\" target=\"_self\"><i class=\"icon fas fa-bug\"></i> Mayor</a>\n";
    }
    $code .= "<a href='/plans/causes/edit/{$cause['cause']}' class='btn btn-sm btn-warning {$status}'><i class=\"icon far fa-edit\"></i> Editar</a>";
    $code .= "<a href='/plans/causes/delete/{$cause['cause']}' class='btn btn-sm btn-danger {$status}'><i class=\"far fa-trash\"></i> Eliminar</a>";
    $code .= "</div>";
    $code .= "</td>";
    $code .= "</tr>";
}
...
$card = $bootstrap->get_Card("card-view-service", array(
    ...
    "header-back" => $back,
    "header-add" => array("href" => "/plans/causes/create/{$oid}", "class" => $status),
    "header-evaluate" => array("href" => "/plans/evaluate/causes/{$oid}", "class" => $status),
    "alert" => array(
        "type" => "info",
...
$info = $bootstrap->get_Card("card-view-service", array(
    "alert" => array(
        'type' => 'secondary',
        'title' => lang("Plans_Causes.info-title"),
        'message' => lang("Plans_Causes.info-message"),
        'class' => 'mb-0'),
));
echo($info);
