...
//[Query]---------------------------------------------------------------------------------------------------------------
$list = $model->get_List($oid, $limit, $offset, $search);
$recordsTotal = $model->get_Total($oid, $search);
//$sql=$model->getLastQuery()->getQuery();
//[Asignations]---------------------------------------------------------------------------------------------------------
$data = array();
$component = '/organization/processes';
foreach ($list as $item) {
    //[Buttons]---------------------------------------------------------------------------------------------------------
    $viewer = "/organization/subprocesses/list/{$item["process"]}";
    $editor = "{$component}/edit/{$item["process"]}";
    $deleter = "{$component}/delete/{$item["process"]}";
    ...
    //[Fields]----------------------------------------------------------------------------------------------------------
    $profile = $mfields->get_Profile($item["responsible"]);
    $row["process"] = $item["process"];
    $row["macroprocess"] = $item["macroprocess"];
    $row["name"] = $item["name"];
    $row["description"] = $strings->get_URLDecode($item["description"]);
    $row["responsible"] = !empty($profile['name']) ? $profile['name'] : lang("App.NotAssigned");
