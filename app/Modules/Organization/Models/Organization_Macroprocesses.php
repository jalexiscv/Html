 ...
 */
class Organization_Macroprocesses extends Model
{
    protected $table = "organization_macroprocesses";
    protected $primaryKey = "macroprocess";
...
//$forge->addKey('profile');
    ...
    public function get_Macroprocess($macroproccess)
    {
        $row = $this->where($this->primaryKey, $macroproccess)->first();
        if (is_array($row)) {
            return ($row);
        } else {
            return (false);
