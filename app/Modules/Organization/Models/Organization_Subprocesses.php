 ...
 */
class Organization_Subprocesses extends Model
{
    protected $table = "organization_subprocesses";
...
//$forge->addKey('profile');
     ...
     */
    public function get_List($process, int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->where("process", $process)
            ->groupStart()
            ->like("subprocess", "%{$search}%")
            ->orLike("name", "%{$search}%")
     ...
     * @return false|array
     */
    public function get_Subprocess($subprocess): false|array
    {
        $result = $this->where("subprocess", $subprocess)->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
     ...
     */
    public function get_SelectData($process): array|false
    {
        $result = $this
     ...
     */
    function get_Total($process, string $search = ""): int
    {
        $result = $this
            ->where("process", $process)
            ->groupStart()
