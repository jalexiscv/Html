...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Social\Models\Social_Modules");
    $mdcxm = model("App\Modules\Social\Models\Social_Clients_Modules");
    $module = 'social';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Social::index');
                $subroutes->add('/home', 'Social::index');
                $subroutes->add('home/(:any)', 'Social::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Social::denied');
                $subroutes->add('(:any)', 'Social::denied');
            }
        );
    }
}
