 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Social\Views\Posts\Delete\form.php]
...
$r = $model->where('post', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Posts.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Posts.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Posts.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
