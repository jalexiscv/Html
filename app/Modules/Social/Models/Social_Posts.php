...
class Social_Posts extends Model
{
    ...
    {
        $key = $this->get_CacheKey("posts-{$limit}-{$offset}-{$search}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $result = $this
                ->groupStart()
                ...
                ->orLike("views", "%{$search}%")
                ->orLike("views_initials", "%{$search}%")
                ->orLike("viewers", "%{$search}%")
                ->orLike("likes", "%{$search}%")
                ->orLike("shares", "%{$search}%")
                ->orLike("comments", "%{$search}%")
                ->orLike("video", "%{$search}%")
                ->orLike("source", "%{$search}%")
                ->orLike("source_alias", "%{$search}%")
    ...
    public function get_Post($post): false|array
    {
        ...
        $dates = service('dates');
        $post = (strlen($post) > 13) ? $this->_get_PostIdBySemantic($post) : $post;
        ...
        if (!$this->is_CacheValid($cache)) {
            ...
            if (is_array($row)) {
                $mfields = model('App\Modules\Social\Models\Social_Users_Fields', true);
                $mattachments = model('App\Modules\Social\Models\Social_Attachments');
                $attachment = $mattachments->get_ByReference($row['post'], 'COVER');
                $row['cover'] = cdn_url($attachment);
                $row['alias'] = $mfields->get_AliasByUser($row['author']);
                $value = $row;
            } else {
                $value = false;
            }
