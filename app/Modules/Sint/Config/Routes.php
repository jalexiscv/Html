...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Sint\Models\Sint_Modules");
    $mdcxm = model("App\Modules\Sint\Models\Sint_Clients_Modules");
    $module = 'sint';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Sint::index');
                $subroutes->add('/home', 'Sint::index');
                $subroutes->add('home/(:any)', 'Sint::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Sint::denied');
                $subroutes->add('(:any)', 'Sint::denied');
            }
        );
    }
}
