...
namespace App\Modules\Sint\Models;
 ...
 */
class Sint_Incidents extends Model
{
    protected $table = "sint_incidents";
    protected $primaryKey = "incident";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'incident' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'case' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
...
//$forge->addKey('profile');
     ...
     * @return array|false
     */
    public function get_Incident($incident): false|array
    {
        $key = $this->get_CacheKey("incident-{$incident}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $incident)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
