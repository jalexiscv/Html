 ...
 */
class Sint_Breaches extends Model
{
    protected $table = "sint_breaches";
    protected $primaryKey = "breach";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'breach' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
...
//$forge->addKey('profile');
     ...
     * @return array|false
     */
    public function get_Breaches($breaches): false|array
    {
        $key = $this->get_CacheKey("breaches-{$breaches}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $breaches)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
