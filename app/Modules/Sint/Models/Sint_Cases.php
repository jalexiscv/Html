 ...
 */
class Sint_Cases extends Model
{
    protected $table = "sint_cases";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'cover' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
...
//$forge->addKey('profile');
     ...
     * @return array|false
     */
    public function get_Case($case): false|array
    {
        $key = $this->get_CacheKey("case-{$case}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $case)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
