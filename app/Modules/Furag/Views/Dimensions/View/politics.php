...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-04-07 04:45:17
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Furag\Views\Politics\List\table.php]
...
//$mpolitics->clear_AllCache();
$rows = $mpolitics->get_CachedSearch($conditions, $limit, $offset, "order ASC");
$total = $mpolitics->get_CountAllResults($conditions);
...
$component = '/furag/politics';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["politic"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["politic"]}";
        $hrefEdit = "$component/edit/{$row["politic"]}";
        $hrefDelete = "$component/delete/{$row["politic"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Furag_Politics.list-title'),
    //"header-back" => $back,
    "header-add" => "/furag/politics/create/{$oid}",
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Furag_Politics.list-title'), "message" => lang('Furag_Politics.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
