...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Furag\Views\Questions\Delete\form.php]
...
$r= $model->getQuestion($oid);
$name = urldecode($r["statement"]);
$message=sprintf(lang("Questions.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Questions.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Questions.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
