...
/**
* █ ---------------------------------------------------------------------------------------------------------------------
* █ ░FRAMEWORK                                  2025-04-07 05:30:23
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Furag\Views\Questions\List\table.php]
...
$fields = array(
		 //"question" => lang("App.Question"),
		 //"politic" => lang("App.politic"),
		 //"reference" => lang("App.Reference"),
		 //"statement" => lang("App.Statement"),
...
//$mquestions->clear_AllCache();
$rows = $mquestions->get_CachedSearch($conditions, $limit, $offset, "reference ASC, year DESC");
$total = $mquestions->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
		array("content" => "#", "class" => "text-center	align-middle"),
		 array("content" => lang("App.Question"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.politic"), "class" => "text-center	align-middle"),
		 array("content" => lang("App.Reference"), "class" => "text-center	align-middle"),
         array("content" => lang("App.Year"), "class" => "text-center	align-middle"),
		 array("content" => lang("App.Statement"), "class" => "text-center	align-middle"),
...
$component = '/furag/questions';
$count = $offset;
foreach ($rows["data"] as $row) {
		if(!empty($row["question"])){
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
				$hrefView="$component/view/{$row["question"]}";
				$hrefEdit="$component/edit/{$row["question"]}";
				$hrefDelete="$component/delete/{$row["question"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
				...
				$bgrid->add_Row(
						array(
								array("content" => $count, "class" => "text-center align-middle"),
								 array("content" => $row['question'], "class" => "text-left align-middle"),
								 //array("content" => $row['politic'], "class" => "text-left align-middle"),
								 array("content" => $row['reference'], "class" => "text-left align-middle"),
                                    array("content" => $row['year'], "class" => "text-left align-middle"),
								 array("content" => $row['statement'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
		"header-title" =>lang('Furag_Questions.list-title'),
		//"header-back" => $back,
		"header-add"=>"/furag/questions/create/{$oid}",
		"alert" => array(
			"icon" => ICON_INFO,
			"type" => "info",
			"title" => lang('Furag_Questions.list-title'),
			"message" => lang('Furag_Questions.list-description')
		),
		"content" => $bgrid,
));
echo($card);
