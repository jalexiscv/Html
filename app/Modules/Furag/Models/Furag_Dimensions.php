...
namespace App\Modules\Furag\Models;
...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mdimensions, esta deberá ser igualada a  model('App\Modules\Furag\Models\Furag_Dimensions');
* @Instruction $mdimensions = model('App\Modules\Furag\Models\Furag_Dimensions');
...
*/
class Furag_Dimensions extends CachedModel
	 {
		 protected $table = "furag_dimensions";
		...
		private function exec_Migrate():void
		{
				...
				try {
						$migrations->setNamespace('App\Modules\Furag');// Set the namespace for the current module
...
/**
		 ...
		 * @return array|false
		 */
		 public function getDimension($dimension):false|array
		{
				$result = parent::getCached($dimension);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
