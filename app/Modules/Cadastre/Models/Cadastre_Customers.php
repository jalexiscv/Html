...
class Cadastre_Customers extends Model
{
    protected $table = "cadastre_customers";
...
//$forge->addKey('profile');
    ...
    public function get_CustomersCensus()
    {
        // Create a subquery to get the latest created_at for each registration
        $subquery = $this->db->table('cadastre_profiles sub_profiles')
            ->select('MAX(created_at) as latest_created_at')
            ->where('sub_profiles.registration = cadastre_profiles.registration', null, false)
            ->groupBy('sub_profiles.registration')
            ->getCompiledSelect();

        // Main query to retrieve the desired data
        $query = $this->select([
            'cadastre_customers.registration',
            'cadastre_customers.customer',
            'cadastre_profiles.*',
        ])
            ->join('cadastre_profiles', 'cadastre_customers.registration = cadastre_profiles.registration', 'inner')
            ->where('cadastre_profiles.anotation', 'CENSUS')
            ->where("cadastre_profiles.created_at = ($subquery)", null, false)
            ->findAll();
    ...
    public function get_CustomersCensusOLD()
    {
        $query = $this->select([
            'cadastre_customers.registration',
            'cadastre_customers.customer',
            ...
            ->join('cadastre_profiles', 'cadastre_customers.registration = cadastre_profiles.registration', 'inner')
            ->where('cadastre_profiles.anotation', 'CENSUS')
            ->where('cadastre_profiles.created_at is NOT NULL', NULL, FALSE)
            ->groupBy('cadastre_profiles.registration')
            ->orderBy('cadastre_profiles.created_at', 'DESC')
            ->findAll();
     ...
     */
    public function get_ListWithGeoreference2($limit, $offset, $route, $search = "")
    {
        $query = $this->select([
            'cadastre_customers.registration',
            'cadastre_customers.customer',
            'cadastre_profiles.profile',
            'cadastre_profiles.registration AS registration_profiles',
            'cadastre_profiles.names',
            'cadastre_profiles.firstname',
            'cadastre_profiles.lastname',
            'cadastre_profiles.cycle',
            'cadastre_profiles.address',
            'cadastre_profiles.reading_route',
            'cadastre_profiles.route_sequence',
            'cadastre_profiles.status',
            'georef_subquery.georeference',
            'georef_subquery.longitude_decimal',
            'georef_subquery.latitude_decimal',
            'georef_subquery.date',
            'georef_subquery.time',
            'georef_subquery.created_at_georeference',
        ])
            ->join('cadastre_profiles', 'cadastre_customers.registration = cadastre_profiles.registration', 'inner')
            ->join(
                '(SELECT registration, MAX(date) AS date, MAX(time) AS time, MAX(longitude_decimal) AS longitude_decimal,
                MAX(latitude_decimal) AS latitude_decimal, MAX(created_at) AS created_at_georeference,
                georeference
         FROM cadastre_georeferences
         WHERE 
            longitude_decimal IS NOT NULL AND latitude_decimal IS NOT NULL
         GROUP BY registration) AS georef_subquery',
                'cadastre_customers.registration = georef_subquery.registration',
                'left'
            )
            ->where('cadastre_profiles.reading_route', $route)
            ->where('cadastre_profiles.status', 'ACTIVE') // Nueva condición agregada
            ->groupBy('cadastre_profiles.registration')
            ->orderBy('cadastre_customers.registration', 'ASC')
    ...
    public function get_ListFunsionBienPerolento($limit, $offset, $search = "")
    {
        $result = $this
            ->join('cadastre_profiles', 'cadastre_customers.customer = cadastre_profiles.customer')
            ->select('cadastre_customers.*, cadastre_profiles.*')
    ...
    public function get_ListByRouteOLD($limit, $offset, $route, $search = "")
    {
        $result = $this->select('cadastre_customers.customer')
            ->select('cadastre_customers.registration')
            ->select('cadastre_profiles.*')
            ->join('cadastre_profiles', 'cadastre_customers.customer = cadastre_profiles.customer', 'left')
            ->groupStart()
            ->where('cadastre_profiles.created_at', null, false)
            ->orWhere('cadastre_profiles.created_at = (SELECT MAX(created_at) FROM cadastre_profiles AS cp WHERE cp.customer = cadastre_customers.customer)')
            ->groupEnd()
            ->where('cadastre_profiles.reading_route', $route)
            ->findAll($limit, $offset);
    ...
    public function get_ListByRoute($limit, $offset, $route, $search = "")
    {
        $query = $this->select('cadastre_customers.customer')
            ->select('cadastre_profiles.*')
            ->join('cadastre_profiles', 'cadastre_customers.customer = cadastre_profiles.customer', 'left')
            ->whereIn('cadastre_profiles.profile', function ($builder) {
                $builder->selectMax('cp.profile')
                    ->from('cadastre_profiles cp')
                    ->where('cp.customer = cadastre_customers.customer');
            })
            ->where('cadastre_profiles.reading_route', $route)
            ->groupBy('cadastre_customers.customer') // Agrupar por cliente
            ->orderBy('cadastre_profiles.created_at', 'DESC') // Ordenar por fecha de creación descendente
            ->findAll($limit, $offset);
    ...
    public function get_ListByRouteOLD2($limit, $offset, $route, $search = "")
    {
        $query = $this->select('cadastre_customers.customer')
            ->select('cadastre_profiles.*')
            ->join('cadastre_profiles', 'cadastre_customers.customer = cadastre_profiles.customer', 'left')
            ->groupStart()
            ->where('cadastre_profiles.profile = (SELECT MAX(profile) FROM cadastre_profiles AS cp WHERE cp.customer = cadastre_customers.customer)')
            ->groupEnd()
            ->where('cadastre_profiles.reading_route', $route)
            ->groupBy('cadastre_customers.customer') // Agrupar por cliente
            ->orderBy('cadastre_profiles.created_at', 'DESC') // Ordenar por fecha de creación descendente
