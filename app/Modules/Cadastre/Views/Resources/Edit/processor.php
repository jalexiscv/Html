...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Cadastre\Views\Resources\Editor\processor.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$continue = "/cadastre/resources/list/{$d["title"]}/";
$edit = "/cadastre/resources/edit/{$d["resource"]}/";
$asuccess = "cadastre/resources-edit-success-message.mp3";
$anoexist = "cadastre/resources-edit-noexist-message.mp3";
//[Build]---------------------------------------------------------------------------------------------------------------
if (isset($row["resource"])) {
    $file = $request->getFile($f->get_fieldId("file"));
    if ($file->isValid()) {
        if ($d['format'] == "GENIALLY") {
            $c = view('App\Modules\Cadastre\Views\Resources\Edit\Processors\genially', array('resource' => $d));
        } elseif ($d['format'] == "H5P") {
            $c = view('App\Modules\Cadastre\Views\Resources\Edit\Processors\h5p', array('resource' => $d));
        } else {
            $c = view('App\Modules\Cadastre\Views\Resources\Edit\Processors\attachment', array('resource' => $d));
        }
    } else {
        $c = view('App\Modules\Cadastre\Views\Resources\Edit\Processors\general', array('resource' => $d));
    }
} else {
