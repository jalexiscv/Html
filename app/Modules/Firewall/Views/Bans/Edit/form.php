...
//[vars]----------------------------------------------------------------------------------------------------------------
$row = $model->where('ban', $oid)->first();
$r["ban"] = $f->get_Value("ban", $row["ban"]);
$r["ip"] = $f->get_Value("ip", $row["ip"]);
$r["date"] = $f->get_Value("date", service("dates")::get_Date());
$r["time"] = $f->get_Value("time", service("dates")::get_Time());
$r["reason"] = $f->get_Value("reason", $row["reason"]);
$r["redirect"] = $f->get_Value("redirect", $row["redirect"]);
$r["url"] = $f->get_Value("url", $row["url"]);
$r["autoban"] = $f->get_Value("autoban", $row["autoban"]);
$back = "/firewall/bans/list/" . lpk();
//[fields]----------------------------------------------------------------------------------------------------------------
$f->fields["ban"] = $f->get_FieldText("ban", array("value" => $r["ban"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
