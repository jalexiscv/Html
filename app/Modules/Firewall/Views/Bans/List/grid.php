...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-05-16 06:32:31
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Firewall\Views\Bans\List\table.php]
...
$fields = array(
    ...
    //"date" => lang("App.date"),
    //"time" => lang("App.time"),
    //"reason" => lang("App.reason"),
    //"redirect" => lang("App.redirect"),
    //"url" => lang("App.url"),
    //"autoban" => lang("App.autoban"),
...
//$mbans->clear_AllCache();
$rows = $mbans->getGrid($limit, $offset, $field, $search);
//echo($rows['sql']);

$total = $mbans->getGridCount($search);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("App.Ban"), "class" => "text-center	align-middle"),
    ...
    array("content" => lang("App.Reason"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.redirect"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.url"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.autoban"), "class" => "text-center	align-middle"),
...
$component = '/firewall/bans';
$count = $offset;
foreach ($rows as $row) {
    if (!empty($row["ban"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["ban"]}";
        $hrefEdit = "$component/edit/{$row["ban"]}";
        $hrefDelete = "$component/delete/{$row["ban"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                array("content" => $row['ban'], "class" => "text-center align-middle"),
                array("content" => $row['ip'], "class" => "text-center align-middle"),
                array("content" => $row['date'], "class" => "text-center align-middle"),
                array("content" => $row['time'], "class" => "text-center align-middle"),
                array("content" => $row['reason'], "class" => "text-left align-middle"),
                //array("content" => $row['redirect'], "class" => "text-left align-middle"),
                //array("content" => $row['url'], "class" => "text-left align-middle"),
                //array("content" => $row['autoban'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Firewall_Bans.list-title'),
    "header-back" => $back,
    "header-add" => "/firewall/bans/create/" . lpk(),
    "alert" => array(
        "icon" => ICON_INFO,
        "type" => "info",
        "title" => lang('Firewall_Bans.list-title'),
        "message" => lang('Firewall_Bans.list-description')
    ),
    "content" => $bgrid,
));
echo($card);
