...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-05-16 06:30:47
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Firewall\Views\Whitelist\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mwhitelist->clear_AllCache();
$rows = $mwhitelist->getCachedSearch($conditions, $limit, $offset, "id DESC");
$total = $mwhitelist->getCountAllResults($conditions);
...
$card = $bootstrap->get_Card("card-grid", array(
    "title" => lang('Whitelist.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/q10files/import/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Whitelist.list-title'), "message" => lang('Whitelist.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
