...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Firewall\Views\Whitelist\Delete\form.php]
...
$r= $model->getId($oid);
$name = urldecode($r["ip"]);
$message=sprintf(lang("Firewall_Whitelist.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Firewall_Whitelist.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Firewall_Whitelist.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
