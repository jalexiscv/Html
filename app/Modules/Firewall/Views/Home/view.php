...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                                                    2024-02-13 00:06:11
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Firewall\Home\breadcrumb.php]
...
$card = $bootstrap->get_Card2("card-view-Firewall", array(
    "class" => "mb-3",
    "header-title" => lang("Firewall.module") . ": <span class='text-muted'>v{$version}</span>",
    "header-back" => "/",
    "image" => "/themes/assets/images/header/module-firewall.png?v2",
    "image-class" => "img-fluid p-3",
    "content" => lang("Firewall.intro-1")
));
echo($card);

if ($authentication->get_LoggedIn() && $authentication->has_Permission("firewall-access")) {
