 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Firewall\Views\Livetraffic\Delete\form.php]
...
$r = $model->where('traffic', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Firewall.livetraffic-delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Livetraffic.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Livetraffic.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
