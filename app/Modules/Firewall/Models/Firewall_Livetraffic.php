...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mlivetraffic, esta deberá ser igualada a  model('App\Modules\Firewall\Models\Firewall_Livetraffic');
 * @Instruction $mlivetraffic = model('App\Modules\Firewall\Models\Firewall_Livetraffic');
 ...
 */
class Firewall_Livetraffic extends CachedModel
{
    protected $table = "firewall_livetraffic";
    protected $primaryKey = "traffic";
    ...
    private function exec_Migrate(): void
    {
        ...
        try {
            $migrations->setNamespace('App\Modules\Firewall');// Set the namespace for the current module
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("browser_code", "%{$search}%")
            ->orLike("os", "%{$search}%")
            ->orLike("os_code", "%{$search}%")
            ->orLike("device_type", "%{$search}%")
            ->orLike("country", "%{$search}%")
            ->orLike("country_code", "%{$search}%")
            ->orLike("request_uri", "%{$search}%")
            ->orLike("domain", "%{$search}%")
            ->orLike("referer", "%{$search}%")
            ->orLike("bot", "%{$search}%")
            ->orLike("date", "%{$search}%")
            ->orLike("time", "%{$search}%")
            ->orLike("uniquev", "%{$search}%")
     ...
     * @return array|false
     */
    public function getTraffic($traffic): false|array
    {
        $result = parent::getCached($traffic);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
