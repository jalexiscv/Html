...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mbans, esta deberá ser igualada a  model('App\Modules\Firewall\Models\Firewall_Bans');
 * @Instruction $mbans = model('App\Modules\Firewall\Models\Firewall_Bans');
 ...
 */
class Firewall_Bans extends CachedModel
{
    protected $table = "firewall_bans";
    protected $primaryKey = "ban";
     ...
     * @return array|false
     */
    public function getBan($ban): false|array
    {
        $result = parent::getCached($ban);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
