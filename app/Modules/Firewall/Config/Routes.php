...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Firewall\Models\Firewall_Modules");
    $mdcxm = model("App\Modules\Firewall\Models\Firewall_Clients_Modules");
    $module = 'firewall';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Firewall::index');
                $subroutes->add('/home', 'Firewall::index');
                $subroutes->add('home/(:any)', 'Firewall::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Firewall::denied');
                $subroutes->add('(:any)', 'Firewall::denied');
            }
        );
    }
}
