...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Application\Views\Modules\Delete\form.php]
...
$r= $model->getModul($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Plex_Modules.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Plex_Modules.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Plex_Modules.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
