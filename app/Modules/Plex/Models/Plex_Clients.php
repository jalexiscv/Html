...
namespace App\Modules\Plex\Models;
...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mclients, esta deberá ser igualada a  model('App\Modules\Application\Models\Application_Clients');
 * @Instruction $mclients = model('App\Modules\Application\Models\Application_Clients');
 ...
 */
class Plex_Clients extends CachedModel
{
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("2fa", "%{$search}%")
            ->orLike("smtp_host", "%{$search}%")
            ->orLike("smtp_port", "%{$search}%")
            ->orLike("smtp_smtpsecure", "%{$search}%")
            ->orLike("smtp_smtpauth", "%{$search}%")
            ->orLike("smtp_username", "%{$search}%")
            ->orLike("smtp_password", "%{$search}%")
            ->orLike("smtp_from_email", "%{$search}%")
            ->orLike("smtp_from_name", "%{$search}%")
            ->orLike("smtp_charset", "%{$search}%")
     ...
     * @return array|false
     */
    public function getClient($client): false|array
    {
        $result = parent::getCached($client);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
