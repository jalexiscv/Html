...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mmodules, esta deberá ser igualada a  model('App\Modules\Application\Models\Application_Modules');
 * @Instruction $mmodules = model('App\Modules\Application\Models\Application_Modules');
 ...
 */
class Plex_Modules extends CachedModel
{
     ...
     * @return array|false
     */
    public function getModule($module):false|array
    {
        $result = parent::getCached($module);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
