...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Groups\Delete\form.php]
...
$r= $model->getGroup($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Iris_Groups.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Iris_Groups.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Iris_Groups.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
