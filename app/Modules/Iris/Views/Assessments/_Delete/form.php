...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Assessments\Delete\form.php]
...
$r= $model->getAssessment($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Iris_Assessments.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Iris_Assessments.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Iris_Assessments.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
