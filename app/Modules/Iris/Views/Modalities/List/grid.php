...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-10-03 04:43:37
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Modalities\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mmodalities->clear_AllCache();
$rows = $mmodalities->getCachedSearch($conditions, $limit, $offset, "modality DESC");
$total = $mmodalities->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("Iris.Modality"), "class" => "text-center	align-middle"),
...
$component = '/iris/modalities';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["modality"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["modality"]}";
        $hrefEdit = "$component/edit/{$row["modality"]}";
        $hrefDelete = "$component/delete/{$row["modality"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Iris_Modalities.list-title'),
    "header-back" => $back,
    "header-add" => "/iris/modalities/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Iris_Modalities.list-title'), "message" => lang('Iris_Modalities.list-description')),
    "content" => $bgrid,
));
echo($card);
?>