...
/**
* █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-10-03 03:56:55
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Settings\List\table.php]
...
**/
//[models]--------------------------------------------------------------------------------------------------------------
$msettings = model('App\Modules\Iris\Models\Iris_Settings');
//[vars]----------------------------------------------------------------------------------------------------------------
$back= "/iris";
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$msettings->clear_AllCache();
$rows = $msettings->getCachedSearch($conditions, $limit, $offset, "setting DESC");
$total = $msettings->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    //array("content" => lang("App.setting"), "class" => "text-center	align-middle"),
...
$component = '/iris/settings';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["setting"])) {
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["setting"]}";
        $hrefEdit = "$component/edit/{$row["setting"]}";
        $hrefDelete = "$component/delete/{$row["setting"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
				...
				$bgrid->add_Row(
						array(
                            ...
                            //array("content" => $row['setting'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Settings.list-title'),
		"header-back" => $back,
    "header-add" => "/iris/settings/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Settings.list-title'), "message" => lang('Settings.list-description')),
		"content" => $bgrid,
));
echo($card);
?>
