 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Categorys\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mcategories->clear_AllCache();
$rows = $mcategories->getCachedSearch($conditions, $limit, $offset, "category DESC");
$total = $mcategories->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("Iris.Category"), "class" => "text-center	align-middle"),
...
$component = '/iris/categories';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["category"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["category"]}";
        $hrefEdit = "$component/edit/{$row["category"]}";
        $hrefDelete = "$component/delete/{$row["category"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Iris_Categories.list-title'),
    "header-back" => $back,
    "header-add" => "/iris/categories/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Iris_Categories.list-title'), "message" => lang('Iris_Categories.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
