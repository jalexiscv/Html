...
/**
* █ ---------------------------------------------------------------------------------------------------------------------
* █ ░FRAMEWORK                                  2025-04-10 06:51:24
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Images\List\table.php]
...
**/
//[models]--------------------------------------------------------------------------------------------------------------
$mimages = model('App\Modules\Iris\Models\Iris_Images');
//[vars]----------------------------------------------------------------------------------------------------------------
$back= "/iris";
...
$fields = array(
		 ...
		 //"capture_date" => lang("App.capture_date"),
		 //"eye" => lang("App.eye"),
		 //"image_file" => lang("App.image_file"),
		 //"image_quality" => lang("App.image_quality"),
		 //"operator_comments" => lang("App.operator_comments"),
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mimages->clear_AllCache();
$rows = $mimages->get_CachedSearch($conditions,$limit, $offset,"image DESC");
$total = $mimages->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
		array("content" => "#", "class" => "text-center	align-middle"),
		 ...
		 //array("content" => lang("App.study"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.capture_date"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.eye"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.image_file"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.image_quality"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.operator_comments"), "class" => "text-center	align-middle"),
...
$component = '/iris/images';
$count = $offset;
foreach ($rows["data"] as $row) {
		if(!empty($row["image"])){
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
				$hrefView="$component/view/{$row["image"]}";
				$hrefEdit="$component/edit/{$row["image"]}";
				$hrefDelete="$component/delete/{$row["image"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
				...
				$bgrid->add_Row(
						array(
								array("content" => $count, "class" => "text-center align-middle"),
								 ...
								 //array("content" => $row['capture_date'], "class" => "text-left align-middle"),
								 //array("content" => $row['eye'], "class" => "text-left align-middle"),
								 //array("content" => $row['image_file'], "class" => "text-left align-middle"),
								 //array("content" => $row['image_quality'], "class" => "text-left align-middle"),
								 //array("content" => $row['operator_comments'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card("card-grid", array(
		"title" =>lang('Images.list-title'),
		"header-back" => $back,
		"header-add"=>"/sie/q10files/import/" . lpk(),
		"alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Images.list-title'), "message" => lang('Images.list-description')),
		"content" => $bgrid,
));
echo($card);
?>
