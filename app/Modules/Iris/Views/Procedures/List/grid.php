...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-10-03 05:58:33
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Procedures\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mprocedures->clear_AllCache();
$rows = $mprocedures->getCachedSearch($conditions, $limit, $offset, "procedure DESC");
$total = $mprocedures->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("Iris.Procedure"), "class" => "text-center	align-middle"),
    array("content" => lang("Iris.Code"), "class" => "text-center	align-middle"),
    array("content" => lang("Iris.Name"), "class" => "text-center	align-middle"),
...
$component = '/iris/procedures';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["procedure"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["procedure"]}";
        $hrefEdit = "$component/edit/{$row["procedure"]}";
        $hrefDelete = "$component/delete/{$row["procedure"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                array("content" => $row['procedure'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Iris_Procedures.list-title'),
    "header-back" => $back,
    "header-add" => "/iris/procedures/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Iris_Procedures.list-title'), "message" => lang('Iris_Procedures.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
