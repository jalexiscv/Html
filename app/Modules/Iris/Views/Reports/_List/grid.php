...
/**
* █ ---------------------------------------------------------------------------------------------------------------------
* █ ░FRAMEWORK                                  2025-04-10 06:52:02
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Reports\List\table.php]
...
**/
//[models]--------------------------------------------------------------------------------------------------------------
$mreports = model('App\Modules\Iris\Models\Iris_Reports');
//[vars]----------------------------------------------------------------------------------------------------------------
$back= "/iris";
...
$fields = array(
		 //"report" => lang("App.report"),
		 //"study" => lang("App.study"),
		 //"professional" => lang("App.professional"),
		 //"validation_date" => lang("App.validation_date"),
		 //"final_diagnosis" => lang("App.final_diagnosis"),
		 //"observations" => lang("App.observations"),
		 //"is_validated" => lang("App.is_validated"),
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mreports->clear_AllCache();
$rows = $mreports->get_CachedSearch($conditions,$limit, $offset,"report DESC");
$total = $mreports->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
		array("content" => "#", "class" => "text-center	align-middle"),
		 //array("content" => lang("App.report"), "class" => "text-center	align-middle"),
		 ...
		 //array("content" => lang("App.validation_date"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.final_diagnosis"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.observations"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.is_validated"), "class" => "text-center	align-middle"),
...
$component = '/iris/reports';
$count = $offset;
foreach ($rows["data"] as $row) {
		if(!empty($row["report"])){
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
				$hrefView="$component/view/{$row["report"]}";
				$hrefEdit="$component/edit/{$row["report"]}";
				$hrefDelete="$component/delete/{$row["report"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
				...
				$bgrid->add_Row(
						array(
								array("content" => $count, "class" => "text-center align-middle"),
								 //array("content" => $row['report'], "class" => "text-left align-middle"),
								 //array("content" => $row['study'], "class" => "text-left align-middle"),
								 //array("content" => $row['professional'], "class" => "text-left align-middle"),
								 //array("content" => $row['validation_date'], "class" => "text-left align-middle"),
								 //array("content" => $row['final_diagnosis'], "class" => "text-left align-middle"),
								 //array("content" => $row['observations'], "class" => "text-left align-middle"),
								 //array("content" => $row['is_validated'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card("card-grid", array(
		"title" =>lang('Reports.list-title'),
		"header-back" => $back,
		"header-add"=>"/sie/q10files/import/" . lpk(),
		"alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Reports.list-title'), "message" => lang('Reports.list-description')),
		"content" => $bgrid,
));
echo($card);
?>
