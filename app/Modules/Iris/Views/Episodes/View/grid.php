...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-04-10 06:52:17
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Studies\List\table.php]
...
$fields = array(
    ...
    //"study_date" => lang("App.study_date"),
    //"study_type" => lang("App.study_type"),
...
//$mstudies->clear_AllCache();
$rows = $mstudies->get_CachedSearch($conditions, $limit, $offset, "study DESC");
$total = $mstudies->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("Iris.Study"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.episode"), "class" => "text-center	align-middle"),
    array("content" => lang("Iris.Study_Date"), "class" => "text-center	align-middle"),
    array("content" => lang("Iris.Study_Type"), "class" => "text-center	align-middle"),
    array("content" => lang("Iris.Study_Status"), "class" => "text-center	align-middle"),
...
$component = '/iris/studies';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["study"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["study"]}";
        $hrefEdit = "$component/edit/{$row["study"]}";
        $hrefDelete = "$component/delete/{$row["study"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Iris_Studies.list-title'),
    "header-back" => $back,
    "header-add" => "/iris/studies/create/{$oid}",
    "alert" => array(
        "icon" => ICON_INFO,
        "type" => "info",
        "title" => lang('Iris_Studies.list-title'),
        "message" => lang('Iris_Studies.list-description')
    ),
    "content" => $bgrid,
));
echo($card);
