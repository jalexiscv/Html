...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Episodes\Delete\form.php]
...
$r= $model->getEpisode($oid);
$name = urldecode($r["episode"]);
$message=sprintf(lang("Iris_Episodes.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Iris_Episodes.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Iris_Episodes.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
