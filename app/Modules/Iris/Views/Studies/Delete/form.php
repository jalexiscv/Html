...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Studies\Delete\form.php]
...
$r= $model->getStudy($oid);
$name = urldecode($r["study"]);
$message=sprintf(lang("Iris_Studies.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Iris_Studies.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Iris_Studies.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
