...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Mstudies\Delete\form.php]
...
$r= $model->getMstudy($oid);
$name = urldecode($r["short_name"]);
$message=sprintf(lang("Iris_Mstudies.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Iris_Mstudies.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Iris_Mstudies.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
