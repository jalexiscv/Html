...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mspecialties, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Specialties');
* @Instruction $mspecialties = model('App\Modules\Iris\Models\Iris_Specialties');
...
*/
class Iris_Specialties extends CachedModel
	 {
		 protected $table = "iris_specialties";
		 protected $primaryKey = "specialty";
...
/**
		 ...
		 * @return array|false
		 */
		 public function getSpecialty($specialty):false|array
		{
				$result = parent::getCached($specialty);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
