...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mreports, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Reports');
* @Instruction $mreports = model('App\Modules\Iris\Models\Iris_Reports');
...
*/
class Iris_Reports extends CachedModel
	 {
		 protected $table = "iris_reports";
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						...
						->like("report", "%{$search}%")
						->orLike("study", "%{$search}%")
						->orLike("professional", "%{$search}%")
						->orLike("validation_date", "%{$search}%")
						->orLike("final_diagnosis", "%{$search}%")
						->orLike("observations", "%{$search}%")
						->orLike("is_validated", "%{$search}%")
		 ...
		 * @return array|false
		 */
		 public function getReport($report):false|array
		{
				$result = parent::getCached($report);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
