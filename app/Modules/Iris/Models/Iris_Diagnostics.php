...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mdiagnostics, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Diagnostics');
* @Instruction $mdiagnostics = model('App\Modules\Iris\Models\Iris_Diagnostics');
...
*/
class Iris_Diagnostics extends CachedModel
	 {
		 protected $table = "iris_diagnostics";
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						...
						->orLike("result_ia", "%{$search}%")
						->orLike("result", "%{$search}%")
		 ...
		 * @return array|false
		 */
		 public function getDiagnostic($diagnostic):false|array
		{
				$result = parent::getCached($diagnostic);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
