...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mcategories, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Categories');
 * @Instruction $mcategories = model('App\Modules\Iris\Models\Iris_Categories');
...
*/
class Iris_Categories extends CachedModel
	 {
...
/**
    ...
    public function getCategory($category): false|array
		{
            $result = parent::getCached($category);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
