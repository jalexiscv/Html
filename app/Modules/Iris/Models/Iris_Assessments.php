...
/**
* @Copilot Siempre que en el código se cree una variable llamada $massessments, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Assessments');
* @Instruction $massessments = model('App\Modules\Iris\Models\Iris_Assessments');
...
*/
class Iris_Assessments extends CachedModel
	 {
		 protected $table = "iris_assessments";
		 protected $primaryKey = "assessment";
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						->groupStart()
						->like("assessment", "%{$search}%")
						->orLike("image", "%{$search}%")
						->orLike("analysis_date", "%{$search}%")
						->orLike("ai_results", "%{$search}%")
						->orLike("confidence_score", "%{$search}%")
						->orLike("ai_recommendations", "%{$search}%")
		 ...
		 * @return array|false
		 */
		 public function getAssessment($assessment):false|array
		{
				$result = parent::getCached($assessment);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
