...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mimage, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Image');
* @Instruction $mimage = model('App\Modules\Iris\Models\Iris_Image');
...
*/
class Iris_Image extends CachedModel
	 {
		 protected $table = "iris_image";
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						->groupStart()
						->like("image", "%{$search}%")
						->orLike("studies", "%{$search}%")
						->orLike("capture_date", "%{$search}%")
						->orLike("eye", "%{$search}%")
						->orLike("image_file", "%{$search}%")
						->orLike("image_quality", "%{$search}%")
						->orLike("operator_comments", "%{$search}%")
		 ...
		 * @return array|false
		 */
		 public function getImage($image):false|array
		{
				$result = parent::getCached($image);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
