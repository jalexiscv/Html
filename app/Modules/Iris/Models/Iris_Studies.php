...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mstudies, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Studies');
* @Instruction $mstudies = model('App\Modules\Iris\Models\Iris_Studies');
...
*/
class Iris_Studies extends CachedModel
	 {
		 protected $table = "iris_studies";
		 protected $primaryKey = "study";
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						->groupStart()
						->like("study", "%{$search}%")
						->orLike("episode", "%{$search}%")
						->orLike("study_date", "%{$search}%")
						->orLike("study_type", "%{$search}%")
		 ...
		 * @return array|false
		 */
		 public function getStudy($study):false|array
		{
				$result = parent::getCached($study);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
