...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mprocedures, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Procedures');
 * //* @Instruction $mprocedures = model('App\Modules\Iris\Models\Iris_Procedures');
 ...
 */
class Iris_Procedures extends CachedModel
{
    protected $table = "iris_procedures";
     ...
     * @return array|false
     */
    public function getProcedure($procedure): false|array
    {
        $result = parent::getCached($procedure);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
