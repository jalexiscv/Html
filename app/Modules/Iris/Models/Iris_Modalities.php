...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mmodalities, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Modalities');
 * @Instruction $mmodalities = model('App\Modules\Iris\Models\Iris_Modalities');
 ...
 */
class Iris_Modalities extends CachedModel
{
    protected $table = "iris_modalities";
    protected $primaryKey = "modality";
     ...
     * @return array|false
     */
    public function getModality($modality): false|array
    {
        $result = parent::getCached($modality);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
