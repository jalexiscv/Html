...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mmstudies, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Mstudies');
* @Instruction $mmstudies = model('App\Modules\Iris\Models\Iris_Mstudies');
...
*/
class Iris_Mstudies extends CachedModel
	 {
		 protected $table = "iris_mstudies";
		 protected $primaryKey = "mstudy";
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						->groupStart()
						->like("mstudy", "%{$search}%")
						->orLike("loinc_code", "%{$search}%")
						->orLike("short_name", "%{$search}%")
						->orLike("long_name", "%{$search}%")
						->orLike("common_name", "%{$search}%")
						->orLike("coding_system", "%{$search}%")
						->orLike("code_version", "%{$search}%")
						->orLike("category", "%{$search}%")
						->orLike("subcategory", "%{$search}%")
						->orLike("procedure_type", "%{$search}%")
						->orLike("modality", "%{$search}%")
						->orLike("cpt_code", "%{$search}%")
						->orLike("snomed_code", "%{$search}%")
						->orLike("status", "%{$search}%")
						->orLike("replaced_by", "%{$search}%")
						->orLike("patient_instructions", "%{$search}%")
						->orLike("duration_minutes", "%{$search}%")
						->orLike("requires_consent", "%{$search}%")
						->orLike("notes", "%{$search}%")
		 ...
		 */
		 public function getSelectData()
			 {
				 $result = $this->select("`{$this->primaryKey}` AS `value`,`short_name` AS `label`")->findAll();
				 if (is_array($result)) {
					 return ($result);
				 } else {
					 return (false);
		 ...
		 * @return array|false
		 */
		 public function getMstudy($mstudy):false|array
		{
				$result = parent::getCached($mstudy);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
