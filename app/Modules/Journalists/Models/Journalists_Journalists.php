...
namespace App\Modules\Journalists\Models;
...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mjournalists, esta deberá ser igualada a  model('App\Modules\Journalists\Models\Journalists_Journalists');
* @Instruction $mjournalists = model('App\Modules\Journalists\Models\Journalists_Journalists');
...
*/
class Journalists_Journalists extends CachedModel
	 {
		 protected $table = "journalists_journalists";
		 protected $primaryKey = "journalist";
		...
		private function exec_Migrate():void
		{
				...
				try {
						$migrations->setNamespace('App\Modules\Journalists');// Set the namespace for the current module
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						->groupStart()
						->like("journalist", "%{$search}%")
						->orLike("citizenshipcard", "%{$search}%")
		 ...
		 * @return array|false
		 */
		 public function getJournalist($journalist):false|array
		{
				$result = parent::getCached($journalist);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
