...
/**
 * Ej: $model = model('App\Modules\Journalists\Models\Journalists_Medias');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Journalists_Medias extends Model
{
    protected $table = "journalists_medias";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'media' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'name' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'facebook' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'twitter' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'instagram' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'limit' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'nit' => ['type' => 'VARCHAR', 'constraint' => 16, 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->like("media", "%{$search}%")
            ->orLike("name", "%{$search}%")
            ->orLike("facebook", "%{$search}%")
            ->orLike("twitter", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Media($media): false|array
    {
        $key = $this->get_CacheKey("media-{$media}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $media)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
