 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Journalists\Views\Journalists\List\json.php]
...
foreach ($list as $item) {
    //[Buttons]---------------------------------------------------------------------------------------------------------
    $viewer = "{$component}/view/{$item["journalist"]}";
    $editor = "{$component}/edit/{$item["journalist"]}";
    $deleter = "{$component}/delete/{$item["journalist"]}";
    ...
    $details .= "<br>Correo electrónico: {$item["email"]}";
    $attachments = $mattachments->get_AttachmentByObject($item["journalist"]);
    //[Fields]----------------------------------------------------------------------------------------------------------
    $row["journalist"] = $item["journalist"];
    $row["citizenshipcard"] = $item["citizenshipcard"];
    $row["firstname"] = strtoupper(@$item["firstname"]);
    $row["lastname"] = strtoupper(@$item["lastname"]);
    $row["details"] = $details;
    $row["email"] = $item["email"];
    $row["phone"] = $item["phone"];
    $row["media"] = $item["media"];
    $row["photo"] = "<img src=\"".cdn_url($attachments["file"])."\" width=\"100%\" />";
    $row["date"] = $item["date"];
    $row["time"] = $item["time"];
