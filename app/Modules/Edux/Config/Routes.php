...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Edux\Models\Edux_Modules");
    $mdcxm = model("App\Modules\Edux\Models\Edux_Clients_Modules");
    $module = 'edux';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Edux::index');
                $subroutes->add('/home', 'Edux::index');
                $subroutes->add('home/(:any)', 'Edux::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Edux::denied');
                $subroutes->add('(:any)', 'Edux::denied');
            }
        );
    }
}
