...
namespace App\Modules\Edux\Models;
 ...
 */
class Edux_Documents extends Model
{
    protected $table = "edux_documents";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'document' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'appendix' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'reference' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'legal' => ['type' => 'TINYTEXT', 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("document", "%{$search}%")
            ->orLike("appendix", "%{$search}%")
