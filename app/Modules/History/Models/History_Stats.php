...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mstats, esta deberá ser igualada a  model('App\Modules\Application\Models\Application_Stats');
 * @Instruction $mstats = model('App\Modules\Application\Models\Application_Stats');
 ...
 */
class Application_Stats extends CachedModel
{
     ...
     * @return array|false
     */
    public function getStat($stat):false|array
    {
        $result = parent::getCached($stat);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
