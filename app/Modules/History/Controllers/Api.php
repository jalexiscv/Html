...
class Api extends ResourceController
{
    ...
    public function stats(string $format, string $option, string $oid)
    {
        //header("Content-Type: text/json");
        ...
        if ($format == "json") {
            if ($option == 'list') {
                echo(view('App\Modules\History\Views\test\List\json', $data));
            } elseif ($option == 'general') {
                echo(view('App\Modules\History\Views\General\List\json', $data));
            }

        } else {
            return ($this->failNotFound(lang("History.Api-breaches-no-option")));
        }
    }
