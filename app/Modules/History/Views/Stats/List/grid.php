...
$musers = model('App\Modules\History\Models\History_Users');
//[vars]----------------------------------------------------------------------------------------------------------------
$back = "/application";
...
foreach ($rows["data"] as $row) {
    if (!empty($row["stat"])) {
        ...
        //[etc]---------------------------------------------------------------------------------------------------------
        $avatar = "/themes/assets/images/avatar.jpg";
        $fullname = "{$row["user"]}: Desconocido";
        $profile = $musers->getCachedProfile($row["user"]);
        $dump = safe_dump($profile);
        if ($profile) {
            if (isset($profile['avatar']) && !empty($profile['avatar'])) {
                $avatar = $profile['avatar'];
            }
            $fullname = @$profile['user'] . ": " . @$profile['firstname'] . " " . @$profile['lastname'];
        }

        $photo = "<img class=\"img-fluid\" src=\"{$avatar}\" width=\"36\">";
        $reference = "<div>";
        $reference .= "<div>{$row['stat']}</div>";
        $reference .= "<div class=\"small text-muted fs-8\">" . $row['ip'] . "</div>";
        $reference .= "</div>";

        $date = "<div>";
        $date .= "<div>{$row['date']}</div>";
        $date .= "<div class=\"small text-muted fs-8\">" . $row['time'] . "</div>";
        $date .= "</div>";


        $details = "{$row['log']}";
        $details .= "<br><b>Usuario</b>: {$fullname}";
        //$details.="<br>{$dump}";

        $bgrid->add_Row(
            array(
                array("content" => $count, "class" => "text-center align-middle"),
                array("content" => $reference, "class" => "text-center align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Stats.list-title'),
    "header-back" => $back,
