...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Application\Views\Themes\List\json.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
foreach ($list as $item) {
    ...
    $lview->attr('href', '/nexus/styles/list/' . $item["theme"]);
    $lview->attr('target', '_self');
    $lview->content(array($iview, " " . lang("App.Styles")));
    $leditor = HtmlTag::tag('a');
    $leditor->attr('class', 'btn btn-outline-secondary');
    $leditor->attr('href', '/nexus/themes/edit/' . $item["theme"]);
    ...
    $ldeleter->attr('href', '/nexus/themes/delete/' . $item["theme"]);
    ...
    $row["description"] = "<div class=\"description\">" . urldecode($item["description"]) . "</div>";
    $row["date"] = $item["date"];
    $row["time"] = $item["time"];
