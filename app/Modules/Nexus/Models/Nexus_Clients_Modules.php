...
class Nexus_Clients_Modules extends Model
{
    ...
    public function regenerate()
    {
        if (!$this->tableExists($this->table)) {
            ...
            $fields = [
                'authorization' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
    ...
    {
        return md5(APPNODE . '_' . $this->version . $this->table . '_' . str_replace('\\', '_', get_class($this)) . '_' . $id);
    }
    ...
    /**
     * Retorna si el cliente tiene o no autorizacion para acceder al modulo, toca de este modo y no con un boleano
     * debido a que el cache, no almacena el boleano false, ya que solo almacena cadenas y el boleano en cadena es vacio
     * y el vacio no se almacena, o al menos eso es lo que entiendo.
     * @param $client
     * @param $module
     * @return \Higgs\Cache\CacheInterface|mixed|string
     */
    public function get_AuthorizedClientByModule($client, $module)
    {
        $cache_key = $this->get_CacheKey("{$client}{$module}");
        if (!$data = cache($cache_key)) {
            $row = $this->where('client', $client)->where('module', $module)->first();
            $data = (is_array($row)) ? "authorized" : "noauthorized";
