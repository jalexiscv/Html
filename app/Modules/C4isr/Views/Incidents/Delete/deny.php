...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\C4isr\Views\Incidents\Delete\deny.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$insident = $mincidents->find($oid);
$case = $mcases->find($insident['case']);

$continue = "/c4isr/cases/view/{$case['case']}";
if ($authentication->get_LoggedIn()) {
    $smarty->assign("title", lang("Incidents.delete-denied-title"));
    $smarty->assign("message", lang("Incidents.delete-denied-message"));
    $smarty->assign("permissions", $permissions);
    $smarty->assign("continue", $continue);
    $smarty->assign("voice", "c4isr/incidents-delete-denied-message.mp3");
