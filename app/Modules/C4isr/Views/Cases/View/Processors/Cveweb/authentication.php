...
if ($variant == "ftp") {
    $query = "port:21,22 authentication disabled country:{$country} hostname:{$domain}";
} elseif ($variant == "rfb") {
    $query = "authentication disabled RFB 003.008 country:{$country} hostname:{$domain}";
} elseif ($variant == "vnc") {
    $query = "port:5900 authentication disabled country:{$country} hostname:{$domain}";
}
...
if ($total_records > 0) {
    foreach ($obj["matches"] as $key => $value) {
        ...
        $hostnames = implode(",", $value['hostnames']);
        $details = "";
        $details .= "<b>IP</b>:{$value['ip_str']} </br>";
        $details .= "<b>Dominios</b>:{$domains} </br>";
        $details .= "<b>Host Name</b>: {$hostnames}</br>";
        $details .= "<b>Organización</b>: {$value['org']}</br>";
        ...
        if (is_array($vulnerabilities)) {
            ...
            foreach ($vulnerabilities as $key => $vulnerability) {
                ...
                $mvuln .= "<tr>";
                $mvuln .= "<td class=\"w-50\"><a href=\"https://nvd.nist.gov/vuln/detail/{$vuln}\" target=\"_blank\">{$vuln}</a></td>";
                $mvuln .= "<td class=\"w-50 text-right\">{$cvss}</td>";
                $mvuln .= "</tr>";
            }
            $mvuln .= "</table>";
        }
