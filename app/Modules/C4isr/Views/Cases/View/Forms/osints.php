...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\C4isr\Views\Cases\Editor\form.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$r["identifier"] = $f->get_Value("identifier", "CSC");
$r["type"] = $row["type"];
$r["search"] = $f->get_Value("search", "");
$type = $s->get_Strtolower($row["type"]);
$back = "/c4isr/cases/home/{$type}/" . lpk();
$identifiers = array(
    array("value" => "CSC", "label" => "Cédula de ciudadanía"),
    array("value" => "PHONE", "label" => "Numero Telefónico"),
...
$f->fields["case"] = $f->get_FieldView("case", array("value" => $r["case"], "proportion" => "col-xl-2 col-lg-2 col-md-2 col-sm-12 col-12", "readonly" => true));
$f->fields["country"] = $f->get_FieldSelect("country", array("selected" => $r["country"], "data" => $countries, "proportion" => "col-xl-2 col-lg-2 col-md-2 col-sm-12 col-12"));
...
//[Groups]-----------------------------------------------------------------------------
$f->groups["g1"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["case"] . $f->fields["country"] . $f->fields["identifier"] . $f->fields["search"] . $f->fields["submit"])));
//[Build]-----------------------------------------------------------------------------s
if ($type == "osint") {
    $ttype = "Osint";
} else {
...
$smarty->assign("header", "SInts/{$oid}");
