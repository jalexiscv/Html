...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\C4isr\Views\Phones\List\json.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
//[Asignations]-----------------------------------------------------------------------------
$data = array();
$component = '/c4isr/phones';
foreach ($list as $item) {
    //[Buttons]-----------------------------------------------------------------------------
    $viewer = "{$component}/view/{$item["phone"]}";
    $editor = "{$component}/edit/{$item["phone"]}";
    $deleter = "{$component}/delete/{$item["phone"]}";
    ...
    $row["local_number"] = $item["local_number"];
    $row["extension"] = $item["extension"];
    $row["type"] = $item["type"];
    $row["carrier"] = $item["carrier"];
    $row["normalized_number"] = $item["normalized_number"];
