...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\C4isr\Views\Breaches\List\json.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
//[Asignations]-----------------------------------------------------------------------------
$data = array();
$iintrusions = HtmlTag::tag('i', array('class' => 'icon fa-regular fa-bolt'), '');
...
$component = '/c4isr/breaches';
foreach ($list as $item) {
    //[Buttons]-----------------------------------------------------------------------------
    $intrusion = "/c4isr/intrusions/list/{$item["breach"]}";
    $viewer = "{$component}/view/{$item["breach"]}";
    $editor = "{$component}/edit/{$item["breach"]}";
    $deleter = "{$component}/delete/{$item["breach"]}";
    $lintrusion = $bootstrap::get_Link('intrusion', array('href' => $intrusion, 'icon' => ICON_BOLT, 'text' => lang("App.Intrusions"), 'class' => 'btn-fucsia'));
    ...
    $options = $bootstrap::get_BtnGroup('options', array("content" => array($lintrusion, $lviewer, $leditor, $ldeleter)));
    //[Fields]-----------------------------------------------------------------------------
    $row["breach"] = $item["breach"];
    $row["domain"] = $item["domain"];
    $row["reference"] = $item["reference"];
    $row["date"] = $item["date"];
    $row["hacker"] = $item["hacker"];
