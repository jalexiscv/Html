...
class C4isr_Countries extends Model
{
    ...
    public function regenerate()
    {
        if (!$this->tableExists($this->table)) {
            ...
            $fields = [
                ...
                '_iso3' => ['type' => 'TEXT', 'null' => FALSE],
                'codigo' => ['type' => 'TEXT', 'null' => FALSE],
    ...
    public function get_SelectCountryCodes()
    {
        $result = $this->select("`codigo` AS `value`,`name` AS `label`")->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
