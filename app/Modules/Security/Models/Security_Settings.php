...
class Security_Settings extends Model
{
     ...
     *   ];
     */
    private function exec_TableRegenerate(): void
    {
    ...
    {
        if (!empty($name)) {
            $cache_key = $this->get_CacheKey("setting-$name");
            if (!$value = cache($cache_key)) {
                $row = $this->where("name", $name)->orderBy("created_at", "DESC")->first();
                $value = (is_array($row) && isset($row["value"])) ? $row["value"] : "";
                cache()->save($cache_key, $value, $this->cache_time);
            }
            return ($value);
        }
        return (false);
    ...
    public function clear_Cache($id): void
    {
     ...
     * @return false|mixed
     */
    private function get_CachedItem($id): mixed
    {
     ...
     * @return array
     */
    protected function _exec_FindCache(array $data): array
    {
        $id = $data['id'] ?? null;
    ...
    protected function _exec_UpdateCache(array $data): void
    {
        $id = $data['id'] ?? null;
        if ($id !== null) {
     ...
     * @return void
     */
    protected function _exec_DeleteCache(array $data): void
    {
        $id = $data['id'] ?? null;
        if ($id !== null) {
