...
if ($status == "PENDING" || empty($status)) {
    ...
    if (count($actions) > 0) {
        ...
        $alert = array(
            "type" => "info",
            "title" => lang("Mipg_Plans.status-pendding-title"),
            "message" => lang("Mipg_Plans.status-pendding-message") . $btn,
        );
        echo($bootstrap->get_Alert($alert));
    } else {
        $alert = array(
            "type" => "danger",
            "title" => lang("Mipg_Plans.status-no-actions-title"),
            "message" => lang("Mipg_Plans.status-no-actions-message"),
        );
        echo($bootstrap->get_Alert($alert));
    }
