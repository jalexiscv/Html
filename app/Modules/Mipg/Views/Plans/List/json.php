 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Plans\Views\Plans\List\json.php]
...
//[Query]---------------------------------------------------------------------------------------------------------------
$list = $mplans->get_ListByActivity($oid, $limit, $offset, $search);
$recordsTotal = $mplans->get_TotalByActivity($oid, $search);//$sql=$mplans->getLastQuery()->getQuery();
//[Asignations]---------------------------------------------------------------------------------------------------------
$data = array();
$component = '/plans/plans';
foreach ($list as $item) {
    //[Buttons]---------------------------------------------------------------------------------------------------------
    $viewer = "/mipg/plans/view/{$item["plan"]}";
    $editor = "/mipg/plans/edit/{$item["plan"]}";
    $deleter = "/mipg/plans/delete/{$item["plan"]}";
    ...
    //[Fields]----------------------------------------------------------------------------------------------------------
    $row["plan"] = $item["plan"];
    $row["plan_institutional"] = $item["plan_institutional"];
    $row["activity"] = $item["activity"];
    $row["manager"] = $item["manager"];
    $row["manager_subprocess"] = $item["manager_subprocess"];
    $row["manager_position"] = $item["manager_position"];
    $row["order"] = $numbers->pad_LeftWithZeros($item["order"], 4);
    $row["description"] = $strings->get_URLDecode($item["description"]);
    $row["formulation"] = $item["formulation"];
