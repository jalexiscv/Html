...
namespace App\Modules\Tdp\Models;

use App\Modules\Mipg\Models\type;
use Higgs\Model;
use Config\Database;
 ...
 */
class Tdp_Indicators extends Model
{
    protected $table = "tdp_indicators";
    protected $primaryKey = "indicator";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'indicator' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $product = ""): array|false
    {
        $result = $this
            ->where("product", $product)
            ->groupStart()
            ->like("indicator", "%{$search}%")
            ->orLike("product", "%{$search}%")
            ->orLike("order", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Indicator($indicator): false|array
    {
        $key = $this->get_CacheKey("indicator-{$indicator}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $indicator)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
    ...
    public function get_Activity($indicator): false|array
    {
        $result = $this->where("indicator", $indicator)->first();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     * @param type $month
     */
    public function get_Score($indicator)
    {
        return (0);
        /**
         * $scores = model("\App\Modules\Mipg\Models\Mipg_Scores");
         * $score = $scores->where("indicator", $indicator)->OrderBy('score', 'DESC')->first();
    ...
    {
        $result = $this->where('product', $cid)->orderBy("order", "ASC")->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     */
    function get_Total(string $search = "", string $product = ""): int
    {
        $result = $this
            ->where("product", $product)
            ->groupStart()
