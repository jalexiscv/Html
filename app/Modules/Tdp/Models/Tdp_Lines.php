 ...
 */
class Tdp_Lines extends Model
{
    protected $table = "tdp_lines";
...
//$forge->addKey('profile');
     ...
     * @param type $month
     */
    public function get_Score($line)
    {
        return (0);
        /**
         * try {
         * $mdiagnostics = model("\App\Modules\Tdp\Models\Tdp_Diagnostics");
         * $diagnostic = $mdiagnostics->where("line", $line)->orderBy("created_at", "DESC")->first();
     ...
     * @return array|false
     */
    public function get_Line(string $line): array|false
    {
        $result = $this->where("line", $line)->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
     ...
     * @return array|false
     */
    public function get_Lines(string $dimension): array|false
    {
