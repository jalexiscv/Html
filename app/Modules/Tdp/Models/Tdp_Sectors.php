 ...
 */
class Tdp_Sectors extends Model
{
    protected $table = "tdp_sectors";
...
//$forge->addKey('profile');
     ...
     * contenpladas al interior de las categorias que corresponden al sectore
     * seleccionado, este metodo se crea para igualar el resultado generado a la
     * forma en la cual se realiza en el excel.
     * @param type
     * @param type $month
     */
    public function get_Score($sector)
    {
        return (0);
        /**
         * $mprograms = model("\App\Modules\Tdp\Models\Tdp_Programs");
         * $mactivities = model("\App\Modules\Tdp\Models\Tdp_Activities");
         * $mscores = model("\App\Modules\Tdp\Models\Tdp_Scores");
         * $programs = $mprograms->get_ListBySector($sector);
         * if (is_array($programs)) {
         * $count = count($programs);
         * $sum = 0;
         * foreach ($programs as $c) {
         * $score = $mprograms->get_Score($c["program"]);
         * $sum += $score;
         * }
         * if ($count > 0) {
     ...
     * @return false|array
     */
    public function get_Sector($sector): false|array
    {
        $result = $this->where("sector", $sector)->first();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
