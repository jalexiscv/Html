 ...
 */
class Tdp_Diagnostics extends Model
{
    protected $table = "tdp_diagnostics";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'line' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
    ...
    public function get_Score($diagnostic)
    {
         ...
         * $mscores = model("\App\Modules\Disa\Tdp\Tdp_Scores");
         * $sectors = $msectors->get_ListByDiagnostic($diagnostic);
         * $count = count($sectors);
         * $sum = 0;
         * if (is_array($sectors)) {
         * foreach ($sectors as $c) {
         * $score = $msectors->get_Score($c["component"]);
         * $sum += $score;
         * }
         * if ($count > 0) {
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $line = ""): array|false
    {
        $result = $this
            ->where('line', $line)
            ->groupStart()
     ...
     */
    function get_Total(string $search = "", string $line = ""): int
    {
        $result = $this
            ->where('line', $line)
            ->groupStart()
