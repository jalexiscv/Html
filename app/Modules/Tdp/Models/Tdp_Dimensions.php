 ...
 */
class Tdp_Dimensions extends Model
{
    protected $table = "tdp_dimensions";
...
//$forge->addKey('profile');
    ...
    {
        $mlines = model("\App\Modules\Tdp\Models\Tdp_Lines", true);
        $lines = $mlines->get_ListByDimension($dimension);
        $count = count($lines);
        if (is_array($lines) && $count > 0) {
            $sum = 0;
            foreach ($lines as $c) {
                $score = $mlines->get_Score($c["line"]);
                $sum += $score;
            }
