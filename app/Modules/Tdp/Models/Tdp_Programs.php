...
use App\Modules\Tdp\Models\type;
use Higgs\Model;
use Config\Database;
 ...
 */
class Tdp_Programs extends Model
{
    protected $table = "tdp_programs";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'sector' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
     ...
     * @param type $month
     */
    public function get_Score($program)
    {
        return (0);
        /**
         * $mactivities = model("\App\Modules\Tdp\Models\Tdp_Activities", true);
         * $mscores = model("\App\Modules\Tdp\Models\Tdp_Scores", true);
         * $activities = $mactivities->get_ListByProgram($program);
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $sector = ""): array|false
    {
        $result = $this
            ->where('sector', $sector)
            ->groupStart()
    ...
    public function get_ListBySector($sector)
    {
        $result = $this->where("sector", $sector)->orderBy("order", "ASC")->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
    ...
    {
        $key = $this->get_CacheKey("program-{$program}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $program)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
     ...
     */
    function get_Total(string $search = "", string $sector = ""): int
    {
        $result = $this
            ->where('sector', $sector)
            ->groupStart()
