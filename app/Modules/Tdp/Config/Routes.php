...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Tdp\Models\Tdp_Modules");
    $mdcxm = model("App\Modules\Tdp\Models\Tdp_Clients_Modules");
    $module = 'tdp';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Tdp::index');
                $subroutes->add('/home', 'Tdp::index');
                $subroutes->add('home/(:any)', 'Tdp::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Tdp::denied');
                $subroutes->add('(:any)', 'Tdp::denied');
            }
        );
    }
}
