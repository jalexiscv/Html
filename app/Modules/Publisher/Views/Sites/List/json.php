 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Publisher\Views\Sites\List\json.php]
...
//[Asignations]---------------------------------------------------------------------------------------------------------
$data = array();
$component = '/publisher/sites';
foreach ($list as $item) {
    //[Buttons]---------------------------------------------------------------------------------------------------------
    $viewer = "{$component}/view/{$item["site"]}";
    $editor = "{$component}/edit/{$item["site"]}";
    $deleter = "{$component}/delete/{$item["site"]}";
    ...
    $row["max_links"] = $item["max_links"];
    $row["links_type"] = $item["links_type"];
    $row["sponsored"] = $item["sponsored"];
    $row["post_cover"] = $item["post_cover"];
    $row["categories"] = $item["categories"];
    $row["min_traffic"] = $item["min_traffic"];
    $row["max_traffic"] = $item["max_traffic"];
    $row["type"] = $item["type"];
    $row["themes"] = $item["themes"];
    $row["moz_da"] = $item["moz_da"];
    $row["moz_pa"] = $item["moz_pa"];
    $row["moz_links"] = $item["moz_links"];
    $row["moz_rank"] = $item["moz_rank"];
    $row["majestic_cf"] = $item["majestic_cf"];
    $row["majestic_tf"] = $item["majestic_tf"];
    $row["majestic_links"] = $item["majestic_links"];
    $row["majestic_rd"] = $item["majestic_rd"];
    $row["ahrefs_dr"] = $item["ahrefs_dr"];
    $row["ahrefs_bl"] = $item["ahrefs_bl"];
    $row["ahrefs_rd"] = $item["ahrefs_rd"];
    $row["ahrefs_obl"] = $item["ahrefs_obl"];
    $row["ahrefs_otm"] = $item["ahrefs_otm"];
    $row["sistrix"] = $item["sistrix"];
