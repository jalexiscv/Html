...
namespace App\Modules\Publisher\Models;
 ...
 */
class Publisher_Sites extends Model
{
    protected $table = "publisher_sites";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'site' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'url' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'created' => ['type' => 'DATE', 'null' => TRUE],
                'image' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'name' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'description' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'country' => ['type' => 'VARCHAR', 'constraint' => 3, 'null' => FALSE],
                'max_links' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'links_type' => ['type' => 'ENUM', 'constraint' => ['FOLLOW', 'NOFOLLOW', 'SPONSORED'], 'default' => 'FOLLOW'],
                'sponsored' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N'],
                'post_cover' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N'],
                'categories' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N'],
                'min_traffic' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'max_traffic' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'type' => ['type' => 'VARCHAR', 'constraint' => 128, 'null' => FALSE],
                'themes' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'moz_da' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'moz_pa' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'moz_links' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'moz_rank' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'majestic_cf' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'majestic_tf' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'majestic_links' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'majestic_rd' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'ahrefs_dr' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'ahrefs_bl' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'ahrefs_rd' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'ahrefs_obl' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'ahrefs_otm' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'sistrix' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'price' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("max_links", "%{$search}%")
            ->orLike("links_type", "%{$search}%")
            ->orLike("sponsored", "%{$search}%")
            ->orLike("post_cover", "%{$search}%")
            ->orLike("categories", "%{$search}%")
            ->orLike("min_traffic", "%{$search}%")
            ->orLike("max_traffic", "%{$search}%")
            ->orLike("type", "%{$search}%")
            ->orLike("themes", "%{$search}%")
            ->orLike("moz_da", "%{$search}%")
            ->orLike("moz_pa", "%{$search}%")
            ->orLike("moz_links", "%{$search}%")
            ->orLike("moz_rank", "%{$search}%")
            ->orLike("majestic_cf", "%{$search}%")
            ->orLike("majestic_tf", "%{$search}%")
            ->orLike("majestic_links", "%{$search}%")
            ->orLike("majestic_rd", "%{$search}%")
            ->orLike("ahrefs_dr", "%{$search}%")
            ->orLike("ahrefs_bl", "%{$search}%")
            ->orLike("ahrefs_rd", "%{$search}%")
            ->orLike("ahrefs_obl", "%{$search}%")
            ->orLike("ahrefs_otm", "%{$search}%")
            ->orLike("sistrix", "%{$search}%")
            ->orLike("price", "%{$search}%")
