...
//[vars]----------------------------------------------------------------------------------------------------------------
$row = $model->where('shortcut', $oid)->first();
$r["shortcut"] = $f->get_Value("shortcut", $row["shortcut"]);
$r["title"] = $f->get_Value("title", $row["title"]);
$r["subtitle"] = $f->get_Value("subtitle", $row["subtitle"]);
$r["href"] = $f->get_Value("href", $row["href"]);
$r["target"] = $f->get_Value("target", $row["target"]);
$r["icon"] = $f->get_Value("icon", $row["icon"]);
$r["permission"] = $f->get_Value("permission", $row["permission"]);
...
$f->fields["target"] = $f->get_FieldSelect("target", array("selected" => $r["target"], "data" => LIST_TARGETS, "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
...
$card = $bootstrap->get_Card("create", array(
    "title" => lang("Shortcuts.edit-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
