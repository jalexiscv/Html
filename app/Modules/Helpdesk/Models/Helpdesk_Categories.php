...
class Helpdesk_Categories extends Model
{
    ...
    /**
     * Retorna el nombre de un tipo de solicitud
     * @param type $type
     */
    public function get_Name($category)
    {
        $row = $this->where('category', $category)->first();
        if (is_array($row)) {
            return ($row['name']);
        }
        return (false);
     ...
     */
    public function get_SelectData($type)
    {
        $result = $this
            ->select("`category` AS `value`,`name` AS `label`")
            ->where('type', $type)
            ->findAll();
        if (is_array($result)) {
            return ($result);
