...
/**
 * @method where(string $string, $conversation)
 */
class Helpdesk_Conversations extends Model
{
    protected $table = "helpdesk_conversations";
    protected $primaryKey = "conversation";
    ...
    public function regenerate()
    {
        if (!$this->tableExists()) {
            ...
            $fields = [
                'conversation' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                ...
                'priority' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    function get_TotalClosed(): int
    {
        $result = $this->where('status', 'closed')->countAllResults();
        return ($result);
    }

    /**
     * Retorna una conversación de la base de datos
     * @param $conversation
     * @return false|array
     */
    function get_Conversation($conversation): false|array
    {
        $row = $this->where('conversation', $conversation)->first();
        if (is_array($row)) {
            return ($row);
        }
        return (false);
    ...
    protected function _exec_updateCache(array $data)
    {
        $id = $data['id'][0] ?? null;
        if ($id !== null) {
            $updatedData = $this->where($this->primaryKey, $id)->first();
            if ($updatedData) {
