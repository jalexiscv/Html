...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Helpdesk\Models\Helpdesk_Modules");
    $mdcxm = model("App\Modules\Helpdesk\Models\Helpdesk_Clients_Modules");
    $module = 'helpdesk';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Helpdesk::index');
                $subroutes->add('/home', 'Helpdesk::index');
                $subroutes->add('home/(:any)', 'Helpdesk::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Helpdesk::denied');
                $subroutes->add('(:any)', 'Helpdesk::denied');
            }
        );
    }
}
