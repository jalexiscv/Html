 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Helpdesk\Views\Services\Delete\form.php]
...
$r = $model->where('service', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Services.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Services.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Services.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
