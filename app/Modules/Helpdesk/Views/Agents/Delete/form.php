...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-03-20 15:45:32
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Helpdesk\Views\Agents\Delete\form.php]
...
$r = $model->find($oid);
$profile = $mfields->get_Profile($r['user']);
$name = urldecode($profile['name']);
$message = sprintf(lang("Agents.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Agents.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Agents.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
