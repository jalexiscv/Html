...
class Api extends ResourceController
{
    ...
    public function agents(string $format, string $option, string $oid)
    {
        ...
        if ($format == "json") {
            if ($option == 'list') {
                echo(view('App\Modules\Crm\Views\Agents\List\json', $data));
            }
        } else {
    ...
    public function appointments(string $format, string $option, string $oid)
    {
        ...
        if ($format == "json") {
            if ($option == 'list') {
                echo(view('App\Modules\Crm\Views\Appointments\List\json', $data));
            }
        } else {
            return ($this->failNotFound(lang("App.Api-breaches-no-option")));
        }
    ...
    public function tickets(string $format, string $option, string $oid)
    {
        ...
        if ($format == "json") {
            if ($option == 'list') {
                echo(view('App\Modules\Crm\Views\Tickets\List\json', $data));
            }
        } else {
            return ($this->failNotFound(lang("App.Api-breaches-no-option")));
        }
    ...
    public function customers(string $format, string $option, string $oid)
    {
        ...
        if ($format == "json") {
            if ($option == 'list') {
                echo(view('App\Modules\Crm\Views\Customers\List\json', $data));
            }
        } else {
            return ($this->failNotFound(lang("App.Api-breaches-no-option")));
        }
