 ...
 */
class Crm_Agents extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'capacity' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'morning_shift_start' => ['type' => 'TIME', 'null' => FALSE],
                'morning_shift_end' => ['type' => 'TIME', 'null' => FALSE],
                'afternoon_shift_start' => ['type' => 'TIME', 'null' => FALSE],
                'afternoon_shift_end' => ['type' => 'TIME', 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("morning_shift_start", "%{$search}%")
            ->orLike("morning_shift_end", "%{$search}%")
            ->orLike("afternoon_shift_start", "%{$search}%")
            ->orLike("afternoon_shift_end", "%{$search}%")
