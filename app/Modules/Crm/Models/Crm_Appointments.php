 ...
 */
class Crm_Appointments extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'appointment' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'agent' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'customer' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
                'date' => ['type' => 'DATE', 'null' => FALSE],
                'time' => ['type' => 'TIME', 'null' => FALSE],
                'duration' => ['type' => 'INT', 'constraint' => 10, 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("appointment", "%{$search}%")
            ->orLike("agent", "%{$search}%")
            ->orLike("customer", "%{$search}%")
            ->orLike("date", "%{$search}%")
            ->orLike("time", "%{$search}%")
            ->orLike("duration", "%{$search}%")
