...
class Crm_Tickets extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'ticket' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'number' => ['type' => 'INT', 'constraint' => 3, 'null' => FALSE],
                'agent' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'date' => ['type' => 'DATE', 'null' => FALSE],
                'time' => ['type' => 'TIME', 'null' => FALSE],
                'elapsed' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
...
//$forge->addKey('profile');
     ...
     */
    public function get_NextTicketNumber(): int
    {
        // Consulta el valor máximo del campo "number"
        $query = $this->selectMax('number')->get();
    ...
    public function get_LowestNumberTicketWithStatus(string $status = 'WAITING')
    {
        // Consulta para obtener el ticket con el número más bajo con status "WAITING"
        $query = $this->where('status', $status)
            ->orderBy('number', 'asc')
            ->first(); // Obtiene la primera fila que coincide con las condiciones
    ...
    public function get_TicketsByRange($start, $end): array|false
    {
    ...
    public function get_TicketsByAgent(string $agent, int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->where('agent', $agent)
            ->groupStart()
