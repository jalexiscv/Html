 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Project\Views\Projects\Creator\form.php]
 ...
 **/
$r["project"] = $f->get_Value("project", pk());
$r["name"] = $f->get_Value("name");
$r["description"] = $f->get_Value("description");
$r["start"] = $f->get_Value("start", safe_get_date());
$r["end"] = $f->get_Value("end", safe_get_date());
$r["status"] = $f->get_Value("status");
$r["budget"] = $f->get_Value("budget");
...
$r["estimated_end_date"] = $f->get_Value("estimated_end_date");
$back = $f->get_Value("back", $server->get_Referer());
//[fields]----------------------------------------------------------------------------------------------------------------
$f->add_HiddenField("back", $back);
$f->fields["project"] = $f->get_FieldText("project", array("value" => $r["project"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
...
//$f->groups["g7"]=$f->get_Group(array("legend"=>"","fields"=>($f->fields["estimated_end_date"])));
//[buttons]-------------------------------------------------------------------------------------------------------------
$f->groups["gy"] = $f->get_GroupSeparator();
$f->groups["gz"] = $f->get_Buttons(array("fields" => $f->fields["submit"] . $f->fields["cancel"]));
//[build]---------------------------------------------------------------------------------------------------------------
$card = $b->get_Card("create", array(
    "title" => lang("Project_Projects.create-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
