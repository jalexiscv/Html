...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mtasks, esta deberá ser igualada a  model('App\Modules\Project\Models\Project_Tasks');
 * @Instruction $mtasks = model('App\Modules\Project\Models\Project_Tasks');
 ...
 */
class Project_Tasks extends CachedModel
{
    ...
    private function exec_Migrate(): void
    {
        ...
        try {
            $migrations->setNamespace('App\Modules\Project');// Set the namespace for the current module
     ...
     * @return array|false
     */
    public function getTask($task): false|array
    {
        $result = parent::getCached($task);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
    ...
    public function getCountByProject($project): int
    {
        $result = $this->where('project', $project)->countAllResults();
        if (is_int($result)) {
            return $result;
        } else {
            return 0;
    ...
    public function getCountByStatusByProject($status, $project): int
    {
        $result = $this->where('project', $project)->where('status', $status)->countAllResults();
        if (is_int($result)) {
            return $result;
        } else {
            return 0;
