...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Applieds');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Applieds extends Model
{
    protected $table = "sie_applieds";
    protected $primaryKey = "applied";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'applied' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
            ...
            $forge->addUniqueKey(['product', 'discount']); // Clave única compuesta
     ...
     * @return array|false
     */
    public function get_Applied($applied): false|array
    {
        $key = $this->get_CacheKey("applied-{$applied}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $applied)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
    ...
    public function set_Applied($discount, $product): bool
    {
        ...
        if ($result === true) {
            cache()->delete($this->get_CacheKey("applied-" . $product . "x" . $discount));
        }
        return ($result);
    }

    public function delete_Applied($discount, $product): bool
    {
        $result = $this->where("discount", $discount)->where("product", $product)->delete();
        if ($result === true) {
            cache()->delete($this->get_CacheKey("applied-" . $product . "x" . $discount));
        }
        return ($result);
