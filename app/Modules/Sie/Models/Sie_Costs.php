...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mcosts, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Costs');
 * @Instruction $mcosts = model('App\Modules\Sie\Models\Sie_Costs');
 ...
 */
class Sie_Costs extends CachedModel
{
    protected $table = "sie_costs";
    protected $primaryKey = "cost";
    ...
    /**
     * Obtiene el valor delcosto por programa y periodo
     * @param $program
     * @param $period
     * @return false|mixed
     */
    public function getValueForCostByProgramByPeriod($program, $period)
    {
        $result = $this
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("value", "%{$search}%")
            ->orLike("currency", "%{$search}%")
            ->orLike("valid_from", "%{$search}%")
            ->orLike("valid_until", "%{$search}%")
            ->groupEnd()
     ...
     * @return array|false
     */
    public function getCost($cost): false|array
    {
        $result = parent::getCached($cost);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
