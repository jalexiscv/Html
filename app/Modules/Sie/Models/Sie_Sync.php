...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $msync, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Sync');
 * @Instruction $msync = model('App\Modules\Sie\Models\Sie_Sync');
 ...
 */
class Sie_Sync extends CachedModel
{
    protected $table = "sie_sync";
    protected $primaryKey = "sync";
     ...
     * @return array|false
     */
    public function getSync($sync): false|array
    {
        $result = parent::getCached($sync);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
