...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mprograms, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Programs');
 * @Instruction $mprograms = model('App\Modules\Sie\Models\Sie_Programs');
 ...
 */
class Sie_Programs extends CachedModel
{
     ...
     * @return array|false
     */
    public function getProgram($program): false|array
    {
        $result = parent::getCached($program);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
