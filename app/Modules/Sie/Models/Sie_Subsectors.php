...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $msubsectors, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Subsectors');
 * @Instruction $msubsectors = model('App\Modules\Sie\Models\Sie_Subsectors');
 ...
 */
class Sie_Subsectors extends CachedModel
{
    protected $table = "sie_subsectors";
    protected $primaryKey = "subsector";
     ...
     */
    public function get_SelectData($network)
    {
        $result = $this
     ...
     * @return array|false
     */
    public function getSubsector($subsector): false|array
    {
        $result = $this->where("subsector", $subsector)->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
