...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Psychometrics');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Psychometrics extends Model
{
    protected $table = "sie_psychometrics";
    protected $primaryKey = "psychometric";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'psychometric' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'teacher' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'file' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'fulfilled' => ['type' => 'VARCHAR', 'constraint' => 1, 'null' => FALSE],
                'comments' => ['type' => 'TEXT', 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("psychometric", "%{$search}%")
            ->orLike("teacher", "%{$search}%")
            ->orLike("file", "%{$search}%")
            ->orLike("fulfilled", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Psychometric($psychometric): false|array
    {
        $key = $this->get_CacheKey("psychometric-{$psychometric}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $psychometric)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
    ...
    public function get_PsychometricByTeacher($teacher): false|array
    {
