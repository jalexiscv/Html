...
class Sie_Versions extends Model
{
     ...
     */
    public function get_SelectData($grid): array|false
    {
        $result = $this->select("`{$this->primaryKey}` AS `value`,`reference` AS `label`")
            ->where('grid', $grid)
            ->findAll();
        if (is_array($result)) {
            return ($result);
    ...
    public function get_LastVersion($grid): false|array
    {
        $row = $this
            ->where('grid', $grid)
    ...
    public function get_All($grid): false|array
    {
        $row = $this
            ->where('grid', $grid)
            ->findAll();
