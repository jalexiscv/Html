...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mnetworks, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Networks');
 * @Instruction $mnetworks = model('App\Modules\Sie\Models\Sie_Networks');
 ...
 */
class Sie_Networks extends CachedModel
{
    protected $table = "sie_networks";
     ...
     * @return array|false
     */
    public function getNetwork($network): false|array
    {
        $result = parent::getCached($network);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
