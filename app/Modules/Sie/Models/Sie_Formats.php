...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mformats, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Formats');
 * @Instruction $mformats = model('App\Modules\Sie\Models\Sie_Formats');
 ...
 */
class Sie_Formats extends CachedModel
{
    protected $table = "sie_formats";
     ...
     * @return array|false
     */
    public function getFormat($format): false|array
    {
        $result = parent::getCached($format);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
