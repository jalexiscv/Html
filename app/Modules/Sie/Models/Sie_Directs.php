...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mdirects, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Directs');
* @Instruction $mdirects = model('App\Modules\Sie\Models\Sie_Directs');
...
*/
class Sie_Directs extends CachedModel
	 {
		 protected $table = "sie_directs";
		 protected $primaryKey = "direct";
...
/**
		 ...
		 * @return array|false
		 */
		 public function getDirect($direct):false|array
		{
				$result = parent::getCached($direct);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
