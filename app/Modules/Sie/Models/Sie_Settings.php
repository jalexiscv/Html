...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $msettings, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Settings');
 * @Instruction $msettings = model('App\Modules\Sie\Models\Sie_Settings');
 ...
 */
class Sie_Settings extends CachedModel
{
    protected $table = "sie_settings";
     ...
     * @return array|false
     */
    public function getSetting($setting): false|array
    {
        $result = parent::getCached($setting);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
