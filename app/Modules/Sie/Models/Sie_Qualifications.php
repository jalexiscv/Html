...
class Sie_Qualifications extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'weighting' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => TRUE],
    ...
    public function get_LastQualificationByTeacher($qualification): false|array
    {
        $row = $this
            ->where("teacher", $qualification)
            ->orderBy("created_at", "DESC")
            ->first();
        if (is_array($row)) {
            $value = $row;
