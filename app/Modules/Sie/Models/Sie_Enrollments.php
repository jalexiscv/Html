...
/**
 * Ej: $menrollments = model('App\Modules\Sie\Models\Sie_Enrollments');
...
class Sie_Enrollments extends Model
{
     ...
     */
    public function get_List(int $limit, int $offset, mixed $search = ""): array|false
    {
        $result = $this
            ->groupStart()
     ...
     */
    function get_Total(mixed $search = ""): int
    {
        $result = $this
            ->groupStart()
            ->orLike("registration", "%{$search}%")
    ...
    public function get_EnrollmentByStudent($student): false|array
    {
        $result = $this->where('registration', $student)->first();
        if (is_array($result) && $result['registration'] == $student) {
            return ($result);
        } else {
            return (false);
        }
    ...
    /**
     * Obtiene la matricula de un estudiante en un programa específico.
     * @param string $student Identificador del estudiante.
     * @param string $program Identificador del programa.
     * @return array|false Devuelve un arreglo con los datos de la inscripción o false si no se encuentra.
     */
    public function get_EnrollmentByStudentAndProgram($student, $program): false|array
    {
        $result = $this
            ->where('registration', $student)
    ...
    public function get_EnrollmentsByStudent($student): false|array
    {
        $result = $this
            ->where('registration', $student)
            ->orderBy("date", "DESC")
