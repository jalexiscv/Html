...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mgroups, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Groups');
 * @Instruction $mgroups = model('App\Modules\Sie\Models\Sie_Groups');
 ...
 */
class Sie_Groups extends CachedModel
{
    protected $table = "sie_groups";
     ...
     */
    public function getSelectData($institution): array|false
    {
        $result = $this->select("`group` AS `value`,`reference` AS `label`")
     ...
     * @return array|false
     */
    public function getGroup($group): false|array
    {
        $result = parent::getCached($group);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
