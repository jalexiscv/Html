...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mspaces, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Spaces');
 * @Instruction $mspaces = model('App\Modules\Sie\Models\Sie_Spaces');
 ...
 */
class Sie_Spaces extends CachedModel
{
     ...
     * @return array|false
     */
    public function getSpace($space): false|array
    {
        $result = parent::getCached($space);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
