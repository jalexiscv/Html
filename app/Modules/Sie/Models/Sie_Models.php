...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mmodels, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Models');
 * @Instruction $mmodels = model('App\Modules\Sie\Models\Sie_Models');
 * @method insert(array $data, bool $returnID = true, bool $protect = true) : int|string
 * @method update(string $model, array $data) : bool
 * @method delete(string $model, bool $purge = false) : bool
 ...
 */
class Sie_Models extends CachedModel
{
    protected $table = "sie_models";
    ...
    protected $allowedFields = [
        ...
        "uses_credits",
        "hours_per_credit",
        "credit_calculation_formula",
        "requires_theoretical_hours",
        "requires_practical_hours",
        "requires_independent_hours",
        "requires_total_hours",
        "validation_rules",
        "configuration",
        "is_active",
        "created_at",
     ...
     * @param type $model codigo primario del registro a consultar
     * @param type $author codigo del usuario del cual se pretende establecer la autoria
     * @return boolean falso o verdadero segun sea el caso
     */
    public function get_Authority($model, $author): bool
    {
        $row = parent::get_CachedFirst([$this->primaryKey => $model]);
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("regulatory_framework", "%{$search}%")
            ->orLike("uses_credits", "%{$search}%")
            ->orLike("hours_per_credit", "%{$search}%")
            ->orLike("credit_calculation_formula", "%{$search}%")
            ->orLike("requires_theoretical_hours", "%{$search}%")
            ->orLike("requires_practical_hours", "%{$search}%")
            ->orLike("requires_independent_hours", "%{$search}%")
            ->orLike("requires_total_hours", "%{$search}%")
            ->orLike("validation_rules", "%{$search}%")
            ->orLike("configuration", "%{$search}%")
            ->orLike("is_active", "%{$search}%")
            ->groupEnd()
     ...
     * @return array|false
     */
    public function getModel($model): false|array
    {
        $result = parent::getCached($model);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
