...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $minstitutions, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Institutions');
 * @Instruction $minstitutions = model('App\Modules\Sie\Models\Sie_Institutions');
 ...
 */
class Sie_Institutions extends CachedModel
{
     ...
     * @return array|false
     */
    public function getInstitution($institution): false|array
    {
        $result = parent::getCached($institution);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
