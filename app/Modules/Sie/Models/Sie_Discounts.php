...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mdiscounts, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Discounts');
 * @Instruction $mdiscounts = model('App\Modules\Sie\Models\Sie_Discounts');
 ...
 */
class Sie_Discounts extends CachedModel
{
     ...
     * @return array|false
     */
    public function getDiscount($discount): false|array
    {
        $result = $this->where('discount', $discount)->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
    ...
    public function getDiscountsByReference($reference)
    {
        $result = $this
