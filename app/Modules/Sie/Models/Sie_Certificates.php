...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mcertificates, esta deberá ser igualada a  model('App\Modules\Sie\Models\Sie_Certificates');
* @Instruction $mcertificates = model('App\Modules\Sie\Models\Sie_Certificates');
...
*/
class Sie_Certificates extends CachedModel
	 {
		 protected $table = "sie_certificates";
...
/**
		 ...
		 * @return array|false
		 */
		 public function getCertificate($certificate):false|array
		{
				$result = parent::getCached($certificate);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
