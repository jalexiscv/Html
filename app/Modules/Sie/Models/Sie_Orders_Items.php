...
class Sie_Orders_Items extends Model
{
    ...
    public function get_ItemsByOrder($order): false|array
    {
        $items = $this->where('order', $order)->findAll();
        if (is_array($items)) {
            return ($items);
        } else {
            return (false);
    ...
    /**
     * Clona los detalles de un pedido antiguo a uno nuevo y retorna el número de elementos clonados.
     * @param $oldOrder
     * @param $newOrder
     * @return int
     */
    public function cloneDetails($oldOrder, $newOrder): int
    {
        $cloned = 0;
        $items = $this->where('order', $oldOrder)->findAll();
        if (is_array($items)) {
