...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Headquarters');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Headquarters extends Model
{
    protected $table = "sie_headquarters";
    protected $primaryKey = "headquarter";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'address' => ['type' => 'VARCHAR', 'constraint' => 128, 'null' => FALSE],
                'phone' => ['type' => 'VARCHAR', 'constraint' => 16, 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Headquarter($headquarter): false|array
    {
        $key = $this->get_CacheKey("headquarter-{$headquarter}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $headquarter)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
