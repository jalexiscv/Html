...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-10-30 07:50:53
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Formats\Creator\form.php]
...
//[models]--------------------------------------------------------------------------------------------------------------
//$model = model("App\Modules\Sie\Models\Sie_Formats");
//[vars]----------------------------------------------------------------------------------------------------------------
/**
...
$types = array_merge($types, LIST_FORMATS_TYPES);
//[fields]----------------------------------------------------------------------------------------------------------------
$f->add_HiddenField("back", $back);
$f->fields["format"] = $f->get_FieldText("format", array("value" => $r["format"], "proportion" => "col-md-4 col-sm-12 col-12", "readonly" => true));
...
$f->fields["file"] = $f->get_FieldFile("file", array("value" => $r["file"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
...
$f->fields["instructions"] = $f->get_FieldTextArea("instructions", array("value" => $r["instructions"], "proportion" => "col-12"));
...
//[groups]----------------------------------------------------------------------------------------------------------------
$f->groups["g1"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["format"] . $f->fields["type"] . $f->fields["file"])));
...
$f->groups["g4"] = $f->get_Group(array("legend" => "", "fields" => (instructionManagerComponent($f->get_FieldId('instructions')))));
//[buttons]-------------------------------------------------------------------------------------------------------------
$f->groups["gz"] = $f->get_Buttons(array("fields" => $f->fields["submit"] . $f->fields["cancel"]));
//[build]---------------------------------------------------------------------------------------------------------------
$card = $b->get_Card2("create", array(
    "header-title" => lang("Sie_Formats.create-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
