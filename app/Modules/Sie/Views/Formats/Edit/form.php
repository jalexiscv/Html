 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Formats\Editor\form.php]
 ...
 **/
$row = $model->getFormat($oid);
//echo($row['instructions']);
$instructionsData = json_decode($row['instructions'], true);
...
$r["instructions"] = $f->get_Value("instructions", $row["instructions"]);
...
$types = array(array("label" => "Seleccione un tipo", "value" => ""));
...
//[groups]----------------------------------------------------------------------------------------------------------------
$f->groups["g1"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["format"] . $f->fields["type"])));
...
$f->groups["g5"] = $f->get_Group(array("legend" => "", "fields" => (instructionManagerComponent($f->get_FieldId('instructions'), $instructionsData))));
//[buttons]-------------------------------------------------------------------------------------------------------------
//$f->groups["gy"] =$f->get_GroupSeparator();
$f->groups["gz"] = $f->get_Buttons(array("fields" => $f->fields["submit"] . $f->fields["cancel"]));
//[build]---------------------------------------------------------------------------------------------------------------
$card = $bootstrap->get_Card2("create", array(
    "header-title" => lang("Sie_Formats.edit-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
