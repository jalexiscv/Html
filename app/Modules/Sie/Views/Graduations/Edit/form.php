...
$r["graduation"] = $f->get_Value("graduation", $row["graduation"]);
$r["city"] = $f->get_Value("city", $row["city"]);
$r["date"] = $f->get_Value("date", service("dates")::get_Date());
$r["application_type"] = $f->get_Value("application_type", $row["application_type"]);
$r["full_name"] = $f->get_Value("full_name", $row["full_name"]);
...
$r["expedition_place"] = $f->get_Value("expedition_place", $row["expedition_place"]);
$r["address"] = $f->get_Value("address", $row["address"]);
$r["phone_1"] = $f->get_Value("phone_1", $row["phone_1"]);
$r["email"] = $f->get_Value("email", $row["email"]);
$r["phone_2"] = $f->get_Value("phone_2", $row["phone_2"]);
$r["degree"] = $f->get_Value("degree", $row["degree"]);
$r["doc_id"] = $f->get_Value("doc_id", $row["doc_id"]);
$r["highschool_diploma"] = $f->get_Value("highschool_diploma", $row["highschool_diploma"]);
$r["highschool_graduation_act"] = $f->get_Value("highschool_graduation_act", $row["highschool_graduation_act"]);
$r["icfes_results"] = $f->get_Value("icfes_results", $row["icfes_results"]);
$r["saber_pro"] = $f->get_Value("saber_pro", $row["saber_pro"]);
$r["academic_clearance"] = $f->get_Value("academic_clearance", $row["academic_clearance"]);
$r["financial_clearance"] = $f->get_Value("financial_clearance", $row["financial_clearance"]);
$r["graduation_fee_receipt"] = $f->get_Value("graduation_fee_receipt", $row["graduation_fee_receipt"]);
$r["graduation_request"] = $f->get_Value("graduation_request", $row["graduation_request"]);
$r["admin_graduation_request"] = $f->get_Value("admin_graduation_request", $row["admin_graduation_request"]);
...
$f->fields["graduation"] = $f->get_FieldText("graduation", array("value" => $r["graduation"], "proportion" => "col-md-3 col-sm-12 col-12", "readonly" => true));
$f->fields["city"] = $f->get_FieldCitySearch("city", array("value" => $r["city"], "proportion" => "col-md-3 col-sm-12 col-12"));
$f->fields["date"] = $f->get_FieldText("date", array("value" => $r["date"], "proportion" => "col-md-3 col-sm-12 col-12", "readonly" => true));
...
$f->fields["status"] = $f->get_FieldSelect("status", array("selected" => $r["status"], "data" => LIST_GRADUATION_STATUS, "proportion" => "col-md-3 col-12"));
...
//[groups]----------------------------------------------------------------------------------------------------------------
$f->groups["g01"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["graduation"] . $f->fields["city"] . $f->fields["date"] . $f->fields["status"])));
...
$card = $bootstrap->get_Card("create", array(
    "title" => lang("Sie_Graduations.edit-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
