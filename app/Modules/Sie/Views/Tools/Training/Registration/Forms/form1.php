...
//[vars]----------------------------------------------------------------------------------------------------------------
$r["period"] = $f->get_Value("period", "2024B");
...
$pensums = array(array("value" => "", "label" => "Seleccione un curso"),);

$qpensums = $mpensums
        ->where("version", "66F32A4BE55F5")
        ->findAll();
...
$courses = array(array("value" => "", "label" => "Seleccione un horario"));

if (!empty($r["pensum"])) {
    $pensum = $mpensums->where("pensum", $r["pensum"])->first();
    $qcourses = $mcourses->where("module", $pensum['module'])->findAll();
}
...
$f->fields["period"] = $f->get_FieldText("period", array("value" => $r["period"], "proportion" => "col-md-4 col-sm-12 col-12", "readonly" => "true"));
...
$f->fields["linkage_type"] = $f->get_FieldSelect("linkage_type", array("selected" => $r["linkage_type"], "data" => LIST_LINKAGE_TRAINING_TYPES, "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));

$f->fields["pensum"] = $f->get_FieldSelect("pensum", array("selected" => $r["pensum"], "data" => $pensums, "proportion" => "col-12"));
$f->fields["course"] = $f->get_FieldSelect("course", array("selected" => $r["course"], "data" => $courses, "proportion" => "col-12"));
...
$f->groups["g02"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["period"] . $f->fields["linkage_type"] . $f->fields["journey"])));

$f->groups["g03"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["pensum"])));
$f->groups["g04"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["course"])));

//$f->groups["g03"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["program"])));
$f->groups["g05"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["first_name"] . $f->fields["second_name"] . $f->fields["first_surname"] . $f->fields["second_surname"])));
...
$card = $b->get_Card("create", array(
        "title" => "Formación continua - Registro",
        "alert" => array(
                'type' => 'info',
                'title' => "¡Atención!",
                'message' => "Formulario solo valido para el registro de al proceso de formación continua",
        ), "content" => $f,
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var pensumField = document.getElementById('<?php echo($fpensum);?>');

        pensumField.addEventListener('change', function () {
            var pensum = this.value;
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '/sie/api/courses/json/pensum/' + pensum, true);
            xhr.onload = function () {
                if (this.status == 200) {
                    var response = JSON.parse(this.responseText);
                    if (Array.isArray(response.data)) { // Verifica si response.data es un array
                        var courses = document.getElementById('<?php echo($fid);?>_course');
                        courses.innerHTML = '<option value="">Seleccione un curso (Actualizado)</option>';
