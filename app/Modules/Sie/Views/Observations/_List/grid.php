...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-11-13 04:44:03
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Observations\List\table.php]
...
foreach ($rows["data"] as $row) {
    if (!empty($row["observation"])) {
        ...
        $hrefDelete = "$component/delete/{$row["observation"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Observations.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/observations/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Observations.list-title'), "message" => lang('Observations.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
