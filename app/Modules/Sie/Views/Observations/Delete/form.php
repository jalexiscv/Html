 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Observations\Delete\form.php]
...
$r = $model->get_Observation($oid);
$r["back"] = $f->get_Value("back", "{$back}?course={$course}");
$name = urldecode($r["content"]);
$message = sprintf(lang("Sie_Observations.delete-message"), $name);
//[fields]----------------------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Sie_Observations.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Sie_Observations.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
