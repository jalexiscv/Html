...
$bgrid->set_Headers(array(
        ...
        array("content" => "Tipo", "class" => "text-start align-middle"),
...
if (is_array($files)) {
    foreach ($files as $file) {
        ...
        $ago = $dates::get_ElapsedDays($created, $dates::get_Date());

        if (empty($file['author']) || $file['author'] == "anonymous") {
            $authorFullName = "Auto Cargado";
            $responsable = "<b>Responsable</b> : {$authorFullName} ";
        } else {
            $authorFullName = $mfields->get_FullName($file['author']);
            $responsable = "<b>Responsable</b> : {$authorFullName} - {$file['author']}";
        }
        ...
        $options = "";
        $details = "<b>{$type}</b><br>";
        $details .= "<b>Fecha de Cargue</b> : {$file['created_at']}. Hace $ago dias.<br>";
        $details .= $responsable;
        $cell_count = array("content" => $count, "class" => "text-center  align-middle",);
        $cell_type = array("content" => $details, "class" => "text-start  align-middle",);
        ...
        if ($ago > 30) {
            $options .= "<a href=\"#\" data-attachment=\"" . $file["attachment"] . "\" class=\"btn btn-sm btn-danger  btn-delete disabled\"><i class=\"fa-light fa-trash\"></i></a>";
        } else {
    ...
    function refreshGridFiles() {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                table += '<tr>';
                table += '<th>#</th>';
                table += '<th class="text-center align-middle">Tipo</th>';
                table += '<th>Archivo</th>';
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    table += '<tr>';
                    table += '<td>' + (i + 1) + '</td>';
                    table += '<td class="text-center align-middle">' + reference + '</td>';
