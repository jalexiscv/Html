...
$bgrid->set_Headers(array(
        ...
        array("content" => "<i class=\"fa-regular fa-gear\"></i>", "class" => "text-center text-nowrap align-middle"),
));
$count = 0;
...
if (is_array($discounteds)) {
    foreach ($discounteds as $discounted) {
        ...
        $options .= "<a href=\"#\" onclick=\"deleteDiscounted('{$discounted['discounted']}')\" title=\"\" class=\"btn btn-sm btn-danger\"><i class=\"fa-light fa-trash\"></i></a>";
        $options .= "</div>";
        $cell_count = array("content" => $count, "class" => "text-center  align-middle",);
        $cell_discount = array("content" => @$discount['name'], "class" => "text-start  align-middle",);
        $cell_type = array("content" => @$discount['type'], "class" => "text-center  align-middle",);
        $cell_value = array("content" => @$discount['value'], "class" => "text-end  align-middle",);
        $cell_options = array("content" => $options, "class" => "text-center  align-middle",);
        $bgrid->add_Row(array($cell_count, $cell_discount, $cell_type, $cell_value, $cell_options));
    }
} else {
