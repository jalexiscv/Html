...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-08-04 10:00:58
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Sync\Editor\form.php]
...
$f = service("forms", array("lang" => "Sie_Sync."));
//[models]--------------------------------------------------------------------------------------------------------------
//$model = model("App\Modules\Sie\Models\Sie_Sync");
//[vars]----------------------------------------------------------------------------------------------------------------
/**
 ...
 **/
$row = $model->getSync($oid);
$r["sync"] = $f->get_Value("sync", $row["sync"]);
$r["type"] = $f->get_Value("type", $row["type"]);
$r["payload"] = $f->get_Value("payload", $row["payload"]);
...
$f->fields["sync"] = $f->get_FieldText("sync", array("value" => $r["sync"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
$f->fields["type"] = $f->get_FieldText("type", array("value" => $r["type"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
$f->fields["payload"] = $f->get_FieldText("payload", array("value" => $r["payload"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
...
$f->groups["g2"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["status"] . $f->fields["result"] . $f->fields["created_at"])));
...
$card = $bootstrap->get_Card("create", array(
    "title" => lang("Sie_Sync.edit-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
