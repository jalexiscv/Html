...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-08-04 10:00:55
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Sync\List\table.php]
 ...
 **/
//[models]--------------------------------------------------------------------------------------------------------------
$msync = model('App\Modules\Sie\Models\Sie_Sync');
//[vars]----------------------------------------------------------------------------------------------------------------
$back = "/sie";
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$msync->clear_AllCache();
$rows = $msync->getCachedSearch($conditions, $limit, $offset, "sync DESC");
$total = $msync->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    //array("content" => lang("App.sync"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.type"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.payload"), "class" => "text-center	align-middle"),
...
foreach ($rows["data"] as $row) {
    if (!empty($row["sync"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["sync"]}";
        $hrefEdit = "$component/edit/{$row["sync"]}";
        $hrefDelete = "$component/delete/{$row["sync"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                //array("content" => $row['sync'], "class" => "text-left align-middle"),
                //array("content" => $row['type'], "class" => "text-left align-middle"),
                //array("content" => $row['payload'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sync.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/sync/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sync.list-title'), "message" => lang('Sync.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
