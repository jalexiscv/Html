 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Statuses\Delete\form.php]
...
$r = $model->get_Status($oid);
$name = urldecode($r["status"]);
$message = sprintf(lang("Sie_Statuses.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Sie_Statuses.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Sie_Statuses.delete-message"), $name),
    "content" => $f,
    "header-back" => $back,
    "voice" => "sie/status-confirm-delete.mp3",
));
echo($card);
?>
