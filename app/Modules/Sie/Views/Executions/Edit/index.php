 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Executions\Editor\index.php]
...
$data['model'] = model("App\Modules\Sie\Models\Sie_Executions");
$data['permissions'] = array('singular' => 'sie-executions-edit', "plural" => 'sie-executions-edit-all');
...
$author = ($teacher == safe_get_user()) ? true : false;
...
if ($plural || $authority) {
    if (!empty($submited)) {
        ...
        $form = view($form, $data);
        $observations = view($observations, $data);
        $json = array(
            'breadcrumb' => view($breadcrumb, $data),
            'main' => $form . $observations,
            'right' => "",
        );
    }
} else {
