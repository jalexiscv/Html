...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-09-16 16:04:44
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Networks\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mnetworks->clear_AllCache();
$rows = $mnetworks->getCachedSearch($conditions, $limit, $offset, "network DESC");
$total = $mnetworks->getCountAllResults($conditions);
...
$component = '/sie/networks';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["network"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["network"]}";
        $hrefEdit = "$component/edit/{$row["network"]}";
        $hrefDelete = "$component/delete/{$row["network"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sie_Networks.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/networks/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sie_Networks.list-title'), "message" => lang('Sie_Networks.list-description')),
    "content" => $bgrid,
));
echo($card);
?>