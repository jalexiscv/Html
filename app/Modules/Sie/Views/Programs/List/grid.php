...
$fields = array(
    ...
    //"name" => lang("App.name"),
    //"acronym" => lang("App.acronym"),
    //"resolution" => lang("App.resolution"),
    //"resolution_date" => lang("App.resolution_date"),
    //"evaluation" => lang("App.evaluation"),
    //"groups" => lang("App.groups"),
    //"preregistration" => lang("App.preregistration"),
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mprograms->clear_AllCache();
$rows = $mprograms->getCachedSearch($conditions, $limit, $offset, "program DESC");
$total = $mprograms->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    //array("content" => lang("App.resolution"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.resolution_date"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.evaluation"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.groups"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.preregistration"), "class" => "text-center	align-middle"),
...
foreach ($rows["data"] as $row) {
    if (!empty($row["program"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["program"]}";
        $hrefEdit = "$component/edit/{$row["program"]}";
        $hrefDelete = "$component/delete/{$row["program"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                //array("content" => $row['resolution'], "class" => "text-left align-middle"),
                //array("content" => $row['resolution_date'], "class" => "text-left align-middle"),
                //array("content" => $row['evaluation'], "class" => "text-left align-middle"),
                //array("content" => $row['groups'], "class" => "text-left align-middle"),
                //array("content" => $row['preregistration'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Programs.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/programs/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sie_Programs.list-title'), "message" => lang('Sie_Programs.list-description')),
    "content" => $bgrid,
));
echo($card);
?>