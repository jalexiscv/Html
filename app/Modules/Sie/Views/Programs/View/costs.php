...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-07-03 10:55:51
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Costs\List\table.php]
...
$fields = array(
    ...
    //"valid_from" => lang("App.valid_from"),
    //"valid_until" => lang("App.valid_until"),
...
//$mcosts->clear_AllCache();
$rows = $mcosts->getCachedSearch($conditions, $limit, $offset, "cost DESC");
$total = $mcosts->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("App.Cost"), "class" => "text-center	align-middle"),
    ...
    //array("content" => lang("App.currency"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.valid_from"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.valid_until"), "class" => "text-center	align-middle"),
...
foreach ($rows["data"] as $row) {
    if (!empty($row["cost"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["cost"]}";
        $hrefEdit = "$component/edit/{$row["cost"]}";
        $hrefDelete = "$component/delete/{$row["cost"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                array("content" => $row['cost'], "class" => "text-center align-middle"),
                //array("content" => $row['program'], "class" => "text-left align-middle"),
                array("content" => $row['period'], "class" => "text-center align-middle"),
                array("content" => $numbers->to_Currency($row['value']), "class" => "text-end align-middle"),
                //array("content" => $row['currency'], "class" => "text-left align-middle"),
                //array("content" => $row['valid_from'], "class" => "text-left align-middle"),
                //array("content" => $row['valid_until'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sie_Costs.list-title-for-program'),
    "header-add" => "/sie/costs/create/{$oid}",
    //"alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sie_Costs.list-title'), "message" => lang('Costs.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
