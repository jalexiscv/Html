 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Discounts\Editor\index.php]
...
$data['model'] = model("App\Modules\Sie\Models\Sie_Discounts");
$data['permissions'] = array('singular' => 'sie-discounts-view', "plural" => 'sie-discounts-view-all');
...
if ($plural || $authority) {
    if (!empty($submited)) {
        ...
        $discount = view($form, $data);
        $products = view($products, $data);
        $json = array(
            'breadcrumb' => view($breadcrumb, $data),
            'main' => ($discount . $products),
            'right' => ""
        );
    }
} else {
