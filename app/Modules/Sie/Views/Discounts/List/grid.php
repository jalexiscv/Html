...
$fields = array(
    //"discount" => lang("App.discount"),
    //"reference" => lang("App.reference"),
    //"name" => lang("App.name"),
    //"description" => lang("App.description"),
    //"value" => lang("App.value"),
    //"type" => lang("App.type"),
    //"character" => lang("App.character"),
    //"status" => lang("App.status"),
    //"author" => lang("App.author"),
);
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mdiscounts->clear_AllCache();
$rows = $mdiscounts->getCachedSearch($conditions, $limit, $offset, "discount DESC");
$total = $mdiscounts->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    //array("content" => lang("App.discount"), "class" => "text-center	align-middle"),
    ...
    //array("content" => lang("App.character"), "class" => "text-center	align-middle"),
...
foreach ($rows["data"] as $row) {
    if (!empty($row["discount"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["discount"]}";
        $hrefEdit = "$component/edit/{$row["discount"]}";
        $hrefDelete = "$component/delete/{$row["discount"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                //array("content" => $row['discount'], "class" => "text-left align-middle"),
                ...
                //array("content" => $row['character'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sie_Discounts.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/discounts/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sie_Discounts.list-title'), "message" => lang('Sie_Discounts.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
