...
$count = $mcourses->getCountByPeriod($period);

$score = $bootstrap->getScore('score-1', [
    'title' => 'Cursos Activos',
    'value' => $count,
    'subtitle' => "Periodo: <b>" . $period . "</b>",
    'icon' => ICON_STUDENTS,
    'variant' => 'default'
]);
echo($score);
...
// Código dinámico para generar $academicPeriodsData a partir de $periods
...
foreach ($periodsReversed as $index => $period) {
    ...
    // Calcular cambio porcentual comparado con el período anterior
    $change = 0.0;
    if ($index > 0) {
