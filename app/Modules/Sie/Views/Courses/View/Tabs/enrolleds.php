...
if(isset($course["free"])&&$course["free"]=="Y"){
    $executions = $mexecutions
        ->select('MAX(execution) as execution, registration,progress,c1,c2,c3,status')
        ->where('course', $oid)
        ->groupBy(['course', 'registration'])
        ->findAll();
}else{
    $executions = $mexecutions
        ->select('MAX(execution) as execution, registration,progress,c1,c2,c3,status')
            ...
            if (!empty($progress['enrollment'])) {
                ...
                if(empty($istatus)){
                    switch (@$execution["status"]) {
                        case 'INPROGRESS':$istatus = '<span class="badge rounded-pill bg-primary">En curso</span>';break;
                        case 'PENDING':$istatus = '<span class="badge rounded-pill bg-secondary">Pendiente</span>';break;
                        case 'APPROVED':$istatus = '<span class="badge rounded-pill bg-success">Aprobado</span>';break;
                        case 'HOMOLOGATION':$istatus = '<span class="badge rounded-pill bg-success">Aprobado por homologación</span>';break;
                        case 'CANCELED':$istatus = '<span class="badge rounded-pill bg-danger">Cancelado</span>';break;
                        case 'POSTPONED':$istatus = '<span class="badge rounded-pill bg-warning text-dark">Aplazado</span>';break;
                        case 'IMPROVEMENT':$istatus = '<span class="badge rounded-pill bg-warning text-dark">Plan de mejora</span>';break;
                        case 'AGREEMENT':$istatus = '<span class="badge rounded-pill bg-info text-dark">Homologación por convenio</span>';break;
                        default:$istatus = '<span class="badge rounded-pill bg-secondary">Pendiente</span>';break;
                    }
                }
