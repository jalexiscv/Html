...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Certificates\Delete\form.php]
...
$r= $model->getCertificate($oid);
$name = urldecode($r["certificate"]);
$message=sprintf(lang("Sie_Certificates.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Sie_Certificates.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Sie_Certificates.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
