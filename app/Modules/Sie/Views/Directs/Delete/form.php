...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Directs\Delete\form.php]
...
$r= $model->getDirect($oid);
$name = urldecode($r["title"]);
$message=sprintf(lang("Sie_Directs.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Sie_Directs.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Sie_Directs.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
