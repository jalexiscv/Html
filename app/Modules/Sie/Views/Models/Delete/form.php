...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Models\Delete\form.php]
...
$r= $model->getModel($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Sie_Models.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Sie_Models.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Sie_Models.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
