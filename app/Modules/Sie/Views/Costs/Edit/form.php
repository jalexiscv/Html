...
$r["cost"] = $f->get_Value("cost", $row["cost"]);
...
$r["valid_from"] = $f->get_Value("valid_from", $row["valid_from"]);
$r["valid_until"] = $f->get_Value("valid_until", $row["valid_until"]);
...
$card = $bootstrap->get_Card2("create", array(
    "header-title" => lang("Sie_Costs.edit-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
