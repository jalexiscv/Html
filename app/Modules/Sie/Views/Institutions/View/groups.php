...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-04-29 06:15:14
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Groups\List\table.php]
...
//$mgroups->clear_AllCache();
$rows = $mgroups->getCachedSearch($conditions, $limit, $offset, "group DESC");
$total = $mgroups->getCountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    //array("content" => lang("App.institution"), "class" => "text-center	align-middle"),
...
$component = '/sie/groups';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["group"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["group"]}";
        $hrefEdit = "$component/edit/{$row["group"]}";
        $hrefDelete = "$component/delete/{$row["group"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                //array("content" => $row['institution'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sie_Groups.list-title'),
    //"header-back" => $back,
    "header-add" => "/sie/groups/create/{$oid}",
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sie_Groups.list-title'), "message" => lang('Sie_Groups.list-description')),
    "content" => $bgrid,
));
echo($card);
?>