...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$minstitutions->clear_AllCache();
$rows = $minstitutions->getCachedSearch($conditions, $limit, $offset, "institution DESC");
$total = $minstitutions->getCountAllResults($conditions);
...
foreach ($rows["data"] as $row) {
    if (!empty($row["institution"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["institution"]}";
        $hrefEdit = "$component/edit/{$row["institution"]}";
        $hrefDelete = "$component/delete/{$row["institution"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sie_Institutions.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/institutions/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sie_Institutions.list-title'), "message" => lang('Sie_Institutions.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
