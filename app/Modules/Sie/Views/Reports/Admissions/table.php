...
$statusArray = ["ADMITTED", "RE-ENTRY", "HOMOLOGATION"];
...
$code .= "<td class='text-center' title=\"{$n["ID_TIPO_DOCUMENTO"]}\">ID_TIPO_DOCUMENTO</td>\n";
$code .= "<td class='text-center' title=\"{$n["NUM_DOCUMENTO"]}\">NUM_DOCUMENTO</td>\n";
$code .= "<td class='text-center' title=\"{$n["PRO_CONSECUTIVO"]}\">PRO_CONSECUTIVO</td>\n";
$code .= "<td class='text-center' title=\"{$n["ID_MUNICIPIO"]}\">ID_MUNICIPIO</td>\n";
$code .= "<td class='text-center' title=\"\">TELEFONO_CONTACTO</td>\n";
$code .= "<td class='text-center' title=\"\">EMAIL_PERSONAL</td>\n";
$code .= "<td class='text-center' title=\"\">FECHA_NACIMIENTO</td>\n";
$code .= "<td class='text-center' title=\"\">ID_ESTRATO</td>\n";
$code .= "<td class='text-center' title=\"\">ID_BENEFICIARIO_LEY_VETERANOS</td>\n";
...
foreach ($statuses as $status) {
    ...
    if (!empty($registration["registration"])) {
        ...
        $residence_city_name = !empty(@$registration['name']) ? $residence_city['name'] : "";

        $causa_no_acceso = 0;

        $status_registration = @$status['registration'];
        $status_identification_type_ = @$registration['identification_type'];
        $status_identification_number = @$registration['identification_number'];
        $status_snies_id_validation_requisite = @$registration['snies_id_validation_requisite'];
        $agreement = $magreements->get_Agreement(@$registration['agreement']);
        $agreement_textual = @$registration['name'];
        $status_nombres = @$status['first_name'] . " " . @$registration['second_name'];
        $status_apellidos = @$status['first_surname'] . " " . @$registration['second_surname'];

        $gender = @$registration['gender'];
        $birth_date = @$registration['birth_date'];
        $stratum = @$registration['stratum'];

        $status_email = @$registration['email_address'];
        $status_phone = @$registration['phone_number'] . "  " . @$registration['mobile'];
        $status_textual_journey = safe_strtoupper(get_sie_textual_journey(@$registration['journey']));
        $status_textual_status = safe_strtoupper(get_sie_textual_status(@$registration['reference']));
        $author = $mfields->get_Profile(@$registration['author']);
        $status_status_author = safe_strtoupper(@$author['name']);
        $status_program = safe_strtoupper(@$registration['program']);

        $program = $mprograms->getProgram(@$registration['program']);
        $status_program_name = safe_strtoupper(@$program['name']);
        // Formato oficial
        $PRO_CONSECUTIVO = @$program['snies'];
        $ID_MUNICIPIO = "76111";
        $TELEFONO_CONTACTO = $status_phone;
        $EMAIL_PERSONAL = $status_email;
        $FECHA_NACIMIENTO = $birth_date;
        $ID_ESTRATO = $stratum;
        $ID_BENEFICIARIO_LEY_VETERANOS = "5";
        ...
        $code .= "    <td class='text-start text-nowrap'>{$PRO_CONSECUTIVO}</td>";
        $code .= "    <td class='text-start text-nowrap'>{$ID_MUNICIPIO}</td>";
        $code .= "    <td class='text-left text-nowrap'>{$TELEFONO_CONTACTO}</td>";
        ...
        $code .= "    <td class='text-center text-nowrap'>{$ID_BENEFICIARIO_LEY_VETERANOS}</td>";
