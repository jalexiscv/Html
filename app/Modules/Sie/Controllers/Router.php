...
class Router extends Controller
{
    ...
    public function route($component = "home", $view = "index", ...$params)
    {
        ...
        //--------------------------------------------------------------------
        if ($component === 'students' && $view === 'view') {
            $filtersConfig = config('Filters');
            $request = service('request');
            $alias = 'singletab';
            $className = $filtersConfig->aliases[$alias] ?? null;

            if ($className && class_exists($className)) {
                $filterInstance = new $className();
                $result = $filterInstance->before($request);

                // Si el filtro devuelve un objeto de respuesta (como una redirección)
                if ($result instanceof ResponseInterface) {
                    // Enviamos la respuesta manualmente y detenemos la ejecución.
                    $result->send();
                    exit();
                }

                // Si el filtro devuelve un string (como una vista de error)
                if (is_string($result)) {
                    return $result;
                }
            }
        ...
        $class = "\\App\\Modules\\Sie\\Controllers\\" . ucfirst($component);
