...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Disa\Views\Scores\Delete\form.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$r = $model->find($oid);
$name = urldecode(@$r["score"]);
$message = sprintf(lang("Scores.delete-message"), $name);
$cancel = "/disa/mipg/scores/list/{$r["activity"]}";
/*
 * -----------------------------------------------------------------------------
 * [Fields]
 * -----------------------------------------------------------------------------
...
$smarty->assign("header", sprintf(lang("Scores.delete-title"), $name));
$smarty->assign("message", sprintf(lang("Scores.delete-message"), $name));
$smarty->assign("form", $f);
$smarty->assign("continue", null);
$smarty->assign("voice", "disa/scores-delete-errors-message.mp3");
