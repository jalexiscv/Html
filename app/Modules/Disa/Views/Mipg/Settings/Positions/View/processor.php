...
if (isset($row["position"])) {
    ...
    $smarty->assign("title", lang("Disa.positions-view-success-title"));
    $smarty->assign("message", sprintf(lang("Disa.positions-view-success-message"), $d['position']));
    $smarty->assign("edit", base_url("/disa/positions/edit/{$d['position']}/" . lpk()));
    $smarty->assign("continue", base_url("/disa/positions/view/{$d["position"]}/" . lpk()));
    $smarty->assign("voice", "disa/positions-view-success-message.mp3");
    $c = $smarty->view('alerts/success.tpl');
} else {
    $smarty = service("smarty");
    $smarty->set_Mode("bs5x");
    $smarty->assign("title", lang("Disa.positions-view-noexist-title"));
    $smarty->assign("message", lang("Disa.positions-view-noexist-message"));
    $smarty->assign("continue", base_url("/disa/positions"));
    $smarty->assign("voice", "disa/positions-view-noexist-message.mp3");
