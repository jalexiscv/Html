...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Disa\Views\Subprocesses\Delete\form.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$r = $model->find($oid);
$positions = model("App\Modules\Disa\Models\Disa_Positions");
$linkeds = $positions->where("subprocess", $oid)->find();
$name = urldecode($r["name"]);
$cancel = "/disa/settings/subprocesses/list/" . lpk();
...
if (is_array($linkeds) && count($linkeds) > 0) {
    ...
    $message = sprintf(lang("Disa.subprocesses-delete-message"), $name);
    $f->add_HiddenField("pkey", $oid);
    $f->fields["cancel"] = $f->get_Cancel("cancel", array("href" => $cancel, "text" => lang("App.Cancel"), "type" => "secondary", "proportion" => "col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12 padding-right"));
    ...
    $smarty = service("smarty");
    $smarty->set_Mode("bs5x");
    $smarty->assign("header", sprintf(lang("Disa.subprocesses-delete-title"), $name));
    $smarty->assign("message", sprintf(lang("Disa.subprocesses-delete-message"), $name));
    $smarty->assign("form", $f);
    $smarty->assign("continue", null);
    $smarty->assign("voice", "disa/subprocesses-delete-errors-message.mp3");
