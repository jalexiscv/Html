...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Disa\Views\Plans\Editor\index.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$data['permissions'] = array('singular' => 'disa-plans-edit', "plural" => 'disa-plans-edit-all');
$singular = $authentication->has_Permission($data['permissions']['singular']);
$plural = $authentication->has_Permission($data['permissions']['plural']);
$author = $mplans->get_Authority($oid, $authentication->get_User());
$authority = ($singular && $author) ? true : false;
$submited = $request->getPost("submited");
$validator = $component . '\View\validator';
$form = $component . '\View\form';
$deny = $component . '\View\deny';
$score = $component . '\View\score';
$noteam = $component . '\View\noteam';
$nomcause = $component . '\View\nomcause';
$nowhy = $component . '\View\nowhy';
/*
* -----------------------------------------------------------------------------
* [Evaluate]
* -----------------------------------------------------------------------------
...
if ($plural || $authority) {
    if ($manager_subprocess) {
        if ($mscore > 0) {
            if (is_array($whys) && count($whys) > 0) {
                $c = view($form, $data);
            } else {
                $c = view($nowhy, $data);
            }
        } else {
            $c = view($nomcause, $data);
        }
    } else {
        $c = view($noteam, $data);
    }
...
* -----------------------------------------------------------------------------
*/
$header = view($component . '\View\breadcrumb', $data);
