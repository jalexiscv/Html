...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Disa\Views\Causes\Delete\form.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$r = $model->find($oid);
$name = urldecode($r["cause"]);

$plan = $mplans->find($r["plan"]);


$message = sprintf(lang("Disa.causes-delete-message"), $name);
$cancel = "/disa/mipg/plans/causes/list/{$r["plan"]}";
/*
 * -----------------------------------------------------------------------------
 * [Fields]
 * -----------------------------------------------------------------------------
...
history_logger(array(
    ...
    "reference" => "COMPONENT",
    "object" => "PLAN",
    "log" => "Accedió a eliminar la causa <b> {$r["description"]}</b> del  <b>plan de acción</b> <b>{$plan['order']}</b>",
));
...
$smarty->assign("header", sprintf(lang("Disa.causes-delete-title"), $name));
$smarty->assign("message", sprintf(lang("Disa.causes-delete-message"), $name));
$smarty->assign("form", $f);
$smarty->assign("continue", null);
$smarty->assign("voice", "disa/causes-delete-errors-message.mp3");
