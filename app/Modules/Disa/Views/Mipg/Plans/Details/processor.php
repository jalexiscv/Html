...
if (isset($row["plan"])) {
    ...
    $smarty->assign("title", lang("Disa.plans-view-success-title"));
    $smarty->assign("message", sprintf(lang("Disa.plans-view-success-message"), $d['plan']));
    $smarty->assign("edit", base_url("/disa/plans/edit/{$d['plan']}/" . lpk()));
    $smarty->assign("continue", base_url("/disa/plans/view/{$d["plan"]}/" . lpk()));
    $smarty->assign("voice", "disa/plans-view-success-message.mp3");
    $c = $smarty->view('alerts/success.tpl');
} else {
    $smarty = service("smarty");
    $smarty->set_Mode("bs5x");
    $smarty->assign("title", lang("Disa.plans-view-noexist-title"));
    $smarty->assign("message", lang("Disa.plans-view-noexist-message"));
    $smarty->assign("continue", base_url("/disa/plans"));
