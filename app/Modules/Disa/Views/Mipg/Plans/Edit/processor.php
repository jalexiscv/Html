...
if (isset($row["plan"])) {
    ...
    $smarty->assign("continue", base_url("/disa/mipg/plans/list/" . $d["activity"]));
    ...
    history_logger(array(
        ...
        "module" => "DISA",
        "author" => $authentication->get_User(),
        "description" => "El usuario <b>@" . $authentication->get_Alias() . "</b> ha editado el plan de acción <b><a href=\"/disa/mipg/plans/view/{$oid}\" target=\"_blank\">{$oid}</b>.",
        "code" => "",
    ));
} else {
    $smarty = service("smarty");
