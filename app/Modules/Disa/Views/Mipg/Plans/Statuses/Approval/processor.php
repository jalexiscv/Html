...
if (isset($row["status"])) {
    ...
    foreach ($actions as $action) {
        $mstatuses->insert($d = array("status" => pk(), "object" => $action["action"], "value" => "APPROVAL", "observations" => "Estado automático", "author" => $authentication->get_User()));
    }

    $smarty = service("smarty");
    $smarty->set_Mode("bs5x");
    $smarty->assign("title", lang("Disa.statuses-approval-request-success-title"));
    $smarty->assign("message", sprintf(lang("Disa.statuses-approval-request-success-message"), $d['status']));
    //$smarty->assign("edit", base_url("/disa/statuses/edit/{$d['status']}/" . lpk()));
    $smarty->assign("continue", $back);
    $smarty->assign("voice", "disa/statuses-approval-request-success-message.mp3");
