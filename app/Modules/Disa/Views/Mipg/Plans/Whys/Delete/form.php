...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Disa\Views\Whys\Delete\form.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$r = $model->find($oid);
$name = $strings->get_Striptags(urldecode($r["description"]));
$message = sprintf(lang("Disa.whys-delete-message"), $name);
$cancel = "/disa/mipg/plans/whys/list/{$r["cause"]}";
/*
 * -----------------------------------------------------------------------------
 * [Fields]
 * -----------------------------------------------------------------------------
...
$smarty->assign("header", sprintf(lang("Disa.whys-delete-title"), $name));
$smarty->assign("message", sprintf(lang("Disa.whys-delete-message"), $name));
$smarty->assign("form", $f);
$smarty->assign("continue", null);
$smarty->assign("voice", "disa/whys-delete-errors-message.mp3");
