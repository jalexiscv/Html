...
$program = $mprograms->where("program", $oid)->first();
$program_name = urldecode($program["name"]);
...
foreach ($diagnostics as $d) {
    ...
    $graph->content("heatGraph(\"graphic-{$d["diagnostic"]}\",\"#{$d["order"]}\",\"{$score}\");");
    ...
    $smarty->assign("header", "Autodiagnostico");
...
history_logger(array(
    ...
    "description" => "Accedió al listado de autodiagnosticos del programa <b><a href=\"/disa/programs/home/{$oid}\" target=\"_blank\">{$program_name}</a></b>",
    "code" => "",
));
