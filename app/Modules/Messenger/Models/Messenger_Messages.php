...
/**
 * Ej: $model = model('App\Modules\Messenger\Models\Messenger_Messages');
...
class Messenger_Messages extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'message' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'type' => ['type' => 'ENUM', 'constraint' => ['EMAIL', 'SMS', 'PUSH'], 'default' => 'EMAIL',],
                'from' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'to' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'subject' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'content' => ['type' => 'TEXT', 'null' => FALSE],
                'priority' => ['type' => 'ENUM', 'constraint' => ['VERYHIGH', 'HIGH', 'ABOVEMEDIUM', 'MEDIUM', 'LOW', 'VERYLOW', 'OPTIONAL', 'NOTIMPORTANT'], 'default' => 'MEDIUM',],
                'date' => ['type' => 'DATE', 'null' => FALSE],
                'time' => ['type' => 'TIME', 'null' => FALSE],
                'status' => ['type' => 'ENUM', 'constraint' => ['SEND', 'QUEUED', 'SENT', 'DELIVERED', 'READ', 'FAILED', 'DRAFT', 'ARCHIVED', 'DELETED', 'BOUNCED', 'SPAM', 'UNREAD', 'PENDING', 'SCHEDULED', 'REPLIED', 'FORWARDED'], 'default' => 'PENDING',],
     ...
     * @return array|false
     */
    public function get_Message($message): false|array
    {
        $key = $this->get_CacheKey("message-{$message}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $message)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
