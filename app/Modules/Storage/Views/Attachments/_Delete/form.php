...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-02-11 14:42:44
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Storage\Views\Attachments\Delete\form.php]
...
$r = $model->where('attachment', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Storage.attachments-delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Attachments.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Attachments.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
