...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Storage\Models\Storage_Modules");
    $mdcxm = model("App\Modules\Storage\Models\Storage_Clients_Modules");
    $module = 'storage';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Storage::index');
                $subroutes->add('/home', 'Storage::index');
                $subroutes->add('home/(:any)', 'Storage::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Storage::denied');
                $subroutes->add('(:any)', 'Storage::denied');
            }
        );
    }
}
