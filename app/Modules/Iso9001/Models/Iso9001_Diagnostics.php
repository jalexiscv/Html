 ...
 */
class Iso9001_Diagnostics extends Model
{
    protected $table = "iso9001_diagnostics";
...
//$forge->addKey('profile');
     ...
     */
    public function get_List(int $limit, int $offset, string $search = "", string $requirement = ""): array|false
    {
        $result = $this
            ->where('requirement', $requirement)
            ->groupStart()
            ->like("diagnostic", "%{$search}%")
            ->orLike("requirement", "%{$search}%")
            ->orLike("order", "%{$search}%")
            ->orLike("name", "%{$search}%")
     ...
     */
    function get_Total(string $search = "", string $requirement = ""): int
    {
        $result = $this
            ->where('requirement', $requirement)
            ->groupStart()
     ...
     * @param type $month
     */
    public function getScore($diagnostic)
    {
        ...
        $components = $mcomponents->getListByDiagnostic($diagnostic);

        $count = count($components);
        $sum = 0;
        if (is_array($components)) {
            foreach ($components as $c) {
                $score = $mcomponents->getScore($c["component"]);
                $sum += $score;
            }
            if ($count > 0) {
