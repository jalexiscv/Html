 ...
 */
class Iso9001_Components extends Model
{
    protected $table = "iso9001_components";
...
//$forge->addKey('profile');
    ...
    public function getListByDiagnostic($diagnostic)
    {
        $result = $this->where("diagnostic", $diagnostic)->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     * @param type $month
     */
    public function getScore($component)
    {
        ...
        $categories = $mcategories->getListByComponent($component);
        if (is_array($categories)) {
            $count = count($categories);
            $sum = 0;
            foreach ($categories as $c) {
                $score = $mcategories->getScore($c["category"]);
                $sum += $score;
            }
            if ($count > 0) {
