 ...
 */
class Iso9001_Categories extends Model
{
    protected $table = "iso9001_categories";
...
//$forge->addKey('profile');
    ...
    {
        $mactivities = model("\App\Modules\Iso9001\Models\Iso9001_Activities", true);
        $mscores = model("\App\Modules\Iso9001\Models\Iso9001_Scores", true);
        $activities = $mactivities->getListByCategory($category);
        $count = count($activities);
        if (is_array($activities) && $count > 0) {
            $sum = 0;
            foreach ($activities as $a) {
                $score = $mactivities->getScore($a["activity"]);
                $sum += $score;
            }
    ...
    public function getListByComponent($component)
    {
        $result = $this->where("component", $component)->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
