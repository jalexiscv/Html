 ...
 */
class Iso9001_Requirements extends Model
{
    protected $table = "iso9001_requirements";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'requirement' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'parent' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    public function getScore($requirement)
    {
        $mdiagnostics = model("\App\Modules\Iso9001\Models\Iso9001_Diagnostics");
        $diagnostic = $mdiagnostics->where("requirement", $requirement)->orderBy("created_at", "DESC")->first();
        if (is_array($diagnostic)) {
            $score = $mdiagnostics->getScore($diagnostic["diagnostic"]);
            if ($score > 0) {
                return ($score);
            }
        }
     ...
     * @return false|array
     */
    public function get_Requirement($requirement): false|array
    {
        //echo("Buscando el requerimiento {$requirement}...");
        $key = $this->get_CacheKey("{$requirement}");
        //Caches::addMessage('Cache key: ' . $key);
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $ttl = $this->cache_time;
            $result = $this->where("requirement", $requirement)->first();
            $cache = array('value' => $result, 'retrieved' => true, "expire" => time() + $ttl);
            Caches::addMessage('Create', $key, $ttl);
            cache()->save($key, $cache, $ttl);
        }
        $remaining = $cache['expire'] - time();
        Caches::addMessage('Read', $key, $remaining);
        if (isset($cache['value']) && (!empty($cache['value']))) {
            return ($cache['value']);
        }
        return (false);
    }
