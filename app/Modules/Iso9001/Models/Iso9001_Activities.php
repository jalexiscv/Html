...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mactivities, esta deberá ser igualada a  model('App\Modules\Iso9001\Models\Iso9001_Activities');
 * @Instruction $mactivities = model('App\Modules\Iso9001\Models\Iso9001_Activities');
 ...
 */
class Iso9001_Activities extends CachedModel
{
    protected $table = "iso9001_activities";
     ...
     * @return array|false
     */
    public function getActivity($activity):false|array
    {
        $result = parent::getCached($activity);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
    ...
    public function getListByCategory($category):array|false
    {
    ...
    /**
     * Obtiene la puntuación de una actividad dada.
     * @param $activity
     * @return float|false
     */
    public function getScore($activity):float|false
    {
        $result = parent::getCachedFirst([$this->primaryKey => $activity]);
        if (is_array($result)) {
            return ($result["score"]);
        } else {
            return (false);
