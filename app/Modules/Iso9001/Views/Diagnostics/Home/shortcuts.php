...
//[data]---------------------------------------------------------------------------------------------------------------
$diagnostics = $mdiagnostics->where("requirement", $oid)->orderBy("order", "DESC")->findAll();
$requirement = $mrequirements->where("requirement", $oid)->first();
//[vars]----------------------------------------------------------------------------------------------------------------
$back = "/iso9001/requirements/home/" . lpk();

$code = "<div class=\"row\trow-cols-xxl-4 row-cols-xl-3 row-cols-lg-2 row-cols-md-1 row-cols-1\ttext-center shortcuts\">\n";
$count = 0;
foreach ($diagnostics as $diagnostic) {
    $count++;
    $percentage = $mdiagnostics->getScore($diagnostic['diagnostic']);
    ...
    $code .= "<a href=\"/iso9001/components/home/{$diagnostic['diagnostic']}\" class=\"stretched-link\">\n";
...
$title = $strings->get_Striptags($requirement["name"]);
$message = $strings->get_Striptags($requirement["description"]);
//[build]---------------------------------------------------------------------------------------------------------------
$card = $bootstrap->get_Card2("card-view-service", array(
    "header-title" => lang('Diagnostics.list-title'),
    "header-back" => $back,
    //"header-add" => '/iso9001/diagnostics/create/' . $oid,
    "header-list" => '/iso9001/diagnostics/list/' . $oid,
    "alert" => array(
        'type' => 'info',
        'title' => $title,
        'message' => $message
...
$info = $bootstrap->get_Card("card-view-service", array(
    "alert" => array('type' => 'secondary', 'title' => lang("Diagnostics.iso9001-diagnostic-info-title"), 'message' => lang("Diagnostics.iso9001-diagnostic-info-message"), 'class' => 'mb-0'),
));
echo($info);
?>