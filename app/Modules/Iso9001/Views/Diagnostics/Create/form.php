...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-02-15 02:50:30
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iso9001\Views\Diagnostics\Creator\form.php]
...
//[models]--------------------------------------------------------------------------------------------------------------
//$model = model("App\Modules\Iso9001\Models\Iso9001_Diagnostics");
...
$r["requirement"] = $f->get_Value("requirement", $oid);
...
$back = "/iso9001/diagnostics/list/{$oid}";
//[fields]----------------------------------------------------------------------------------------------------------------
...
$f->fields["requirement"] = $f->get_FieldText("requirement", array("value" => $r["requirement"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12", "readonly" => "readonly"));
...
$f->groups["g2"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["requirement"] . $f->fields["name"])));
...
$card = $b->get_Card("create", array(
    ...
    "alert" => array(
        ...
        'message' => sprintf(lang("Diagnostics.iso9001-info-requirement"), "{$requirement["name"]} ({$requirement["description"]})")
    ),
    "content" => $f,
    "header-back" => $back
));
