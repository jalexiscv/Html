...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Iso9001\Models\Iso9001_Modules");
    $mdcxm = model("App\Modules\Iso9001\Models\Iso9001_Clients_Modules");
    $module = 'iso9001';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Iso9001::index');
                $subroutes->add('/home', 'Iso9001::index');
                $subroutes->add('home/(:any)', 'Iso9001::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Iso9001::denied');
                $subroutes->add('(:any)', 'Iso9001::denied');
            }
        );
    }
}
