...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-09-14 03:16:17
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Application\Views\Ia\Messages\Editor\form.php]
...
$f = service("forms", array("lang" => "Ia."));
//[models]--------------------------------------------------------------------------------------------------------------
//$model = model("App\Modules\Application\Models\Application_Ia");
//[vars]----------------------------------------------------------------------------------------------------------------
$row = $model->get_Ia($oid);
$r["message"] = $f->get_Value("message", $row["message"]);
$r["from"] = $f->get_Value("from", $row["from"]);
...
$f->fields["to"] = $f->get_FieldText("to", array("value" => $r["to"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
...
//[groups]----------------------------------------------------------------------------------------------------------------
$f->groups["g1"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["message"] . $f->fields["from"] . $f->fields["to"])));
$f->groups["g2"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["priority"] . $f->fields["content"] . $f->fields["date"])));
