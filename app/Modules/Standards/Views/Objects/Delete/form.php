...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Standards\Views\Objects\Delete\form.php]
...
$r= $mobjects->get_Object($oid);

$name = urldecode($r["name"]);
$message=sprintf(lang("Standards_Objects.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Standards_Objects.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Standards_Objects.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
