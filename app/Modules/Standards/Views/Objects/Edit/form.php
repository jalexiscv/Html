...
$r["attributes"] = $f->get_Value("attributes",$row["attributes"]);
$r["value"] = $f->get_Value("value",$row["value"]);
$r["evaluation"] = $f->get_Value("evaluation",$row["evaluation"]);
$r["type_content"] = $f->get_Value("type_content",$row["type_content"]);
$r["type_node"] = $f->get_Value("type_node",$row["type_node"]);
$r["attachments"] = $f->get_Value("attachments",$row["attachments"]);


$parent = !empty($r["parent"])?$r["parent"] : "";
...
$card = $bootstrap->get_Card2("create", array(
		 "header-title" => lang("Standards_Objects.edit-title"),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
