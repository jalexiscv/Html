...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Standards\Models\Standards_Modules");
    $mdcxm = model("App\Modules\Standards\Models\Standards_Clients_Modules");
    $module = 'standards';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Standards::index');
                $subroutes->add('/home', 'Standards::index');
                $subroutes->add('home/(:any)', 'Standards::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Standards::denied');
                $subroutes->add('(:any)', 'Standards::denied');
            }
        );
    }
}
