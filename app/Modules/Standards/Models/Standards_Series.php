...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mseries, esta deberá ser igualada a  model('App\Modules\Standards\Models\Standards_Series');
 * @Instruction $mseries = model('App\Modules\Standards\Models\Standards_Series');
 ...
 */
class Standards_Series extends CachedModel
{
    ...
    {
        $result = parent::get_Cached($serie);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
