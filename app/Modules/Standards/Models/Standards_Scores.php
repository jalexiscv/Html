...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mscores, esta deberá ser igualada a  model('App\Modules\Standards\Models\Standards_Scores');
 * @Instruction $mscores = model('App\Modules\Standards\Models\Standards_Scores');
 ...
 */
class Standards_Scores extends CachedModel
{
    protected $table = "standards_scores";
    ...
    public function getScoreForObject($object): false|array
    {
    ...
    public function getLastScoreForObject($object): false|array
    {
        $result = $this->where('object', $object)->orderBy('created_at', 'DESC')->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
