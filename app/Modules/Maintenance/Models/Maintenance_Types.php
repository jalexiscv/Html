...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mtypes, esta deberá ser igualada a  model('App\Modules\Maintenance\Models\Maintenance_Types');
* @Instruction $mtypes = model('App\Modules\Maintenance\Models\Maintenance_Types');
...
*/
class Maintenance_Types extends CachedModel
	 {
...
/**
		 ...
		 * @return array|false
		 */
		 public function getType($type):false|array
		{
				$result = parent::getCached($type);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
