...
/**
* @Copilot Siempre que en el código se cree una variable llamada $msheets, esta deberá ser igualada a  model('App\Modules\Maintenance\Models\Maintenance_Sheets');
* @Instruction $msheets = model('App\Modules\Maintenance\Models\Maintenance_Sheets');
...
*/
class Maintenance_Sheets extends CachedModel
	 {
		 protected $table = "maintenance_sheets";
		 protected $primaryKey = "sheet";
...
/**
		 ...
		 * @return array|false
		 */
		 public function getSheet($sheet):false|array
		{
				$result = parent::getCached($sheet);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
     ...
     * @return boolean falso o verdadero segun sea el caso
     */
    public function getAsset($asset, $name): string
    {
        $row = $this->where("asset", $asset)->where("name", $name)->orderBy("created_at", "DESC")->first();
