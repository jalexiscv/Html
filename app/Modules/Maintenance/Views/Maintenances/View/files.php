...
if (is_array($files)) {
    foreach ($files as $file) {
        ...
        foreach (MAINTENANCES_LIST_FILE_TYPES as $fileType) {
...
foreach (MAINTENANCES_LIST_FILE_TYPES as $type) {
    $value = $type["value"];
    $label = $type["label"];
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    options += "</div>";
                    const MAINTENANCES_LIST_FILE_TYPES = {
                        <?php foreach (MAINTENANCES_LIST_FILE_TYPES as $type) {?>
                        <?php echo("\"{$type['value']}\":\"{$type['label']}\",");?>
                        <?php }?>
                    };
                    let reference = MAINTENANCES_LIST_FILE_TYPES[files[i].reference] || "Otro";
                    table += '<tr>';
                    table += '<td>' + (i + 1) + '</td>';
