...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-04-08 11:49:20
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Maintenance\Views\Maintenances\List\table.php]
...
$fields = array(
    "general" => "General",
    //"asset" => lang("App.asset"),
    //"type" => lang("App.type"),
    //"scheduled" => lang("App.scheduled"),
...
//[build]--------------------------------------------------------------------------------------------------------------
//$mmaintenances->clear_AllCache();
$rows = $mmaintenances->getGrid($limit, $offset, $field, $search);
$total = $mmaintenances->getGridCount($search);
...
$bgrid->set_Headers(array(
    ...
    array("content" => "Mantenimiento", "class" => "text-center	align-middle"),
    array("content" => "Detalles", "class" => "text-center	align-middle"),
    //array("content" => "Activo", "class" => "text-center	align-middle"),
    //array("content" => lang("App.type"), "class" => "text-center	align-middle"),
    //array("content" => "Programado", "class" => "text-center	align-middle"),
    //array("content" => "Ejecutado", "class" => "text-center	align-middle"),
...
$component = '/maintenance/maintenances';
$count = $offset;
foreach ($rows as $row) {
    if (!empty($row["maintenance"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["maintenance"]}";
        $hrefEdit = "$component/edit/{$row["maintenance"]}";
        $hrefDelete = "$component/delete/{$row["maintenance"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        //[etc]---------------------------------------------------------------------------------------------------------
        $typo=safe_strtoupper($mmaintenances->getLabelType($row['type']));
        $status=safe_strtoupper($mmaintenances->getLabelStatus($row['status']));
        //[---]---------------------------------------------------------------------------------------------------------
        $asset=$massets->getAsset($row['asset']);
        $details="<b>Activo:</b> ".@$asset["name"]." - ".@$row['asset']."<br>";
        $details.="<b>Tipo:</b> {$typo}<br>";
        $details.="<b>Programado:</b> {$row['scheduled']}<br>";
        $details.="<b>Ejecutado:</b> {$row['execution']}<br>";
        $details.="<b>Responsable:</b> {$row['responsible']}<br>";
        $details.="<b>Estado:</b> {$status}<br>";
        $details.="<b>Descripción:</b> {$status}<br>";
        ...
        $bgrid->add_Row(
            array(
                ...
                //array("content" => $row['scheduled'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Maintenance_Maintenances.list-title'),
    "header-back" => $back,
    //"header-add" => "/maintenance/maintenances/create/" . lpk(),
    "alert" => array(
        "icon" => ICON_INFO,
        "type" => "info",
        "title" => lang('Maintenance_Maintenances.list-title'),
        "message" => lang('Maintenance_Maintenances.list-description')
    ),
    "content" => $bgrid,
));
echo($card);
