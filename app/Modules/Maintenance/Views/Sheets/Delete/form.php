...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Maintenance\Views\Sheets\Delete\form.php]
...
$r= $model->getSheet($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Maintenance_Sheets.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Maintenance_Sheets.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Maintenance_Sheets.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
