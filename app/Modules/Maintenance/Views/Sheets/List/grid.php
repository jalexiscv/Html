...
/**
* █ ---------------------------------------------------------------------------------------------------------------------
* █ ░FRAMEWORK                                  2025-04-08 11:56:20
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Maintenance\Views\Sheets\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$msheets->clear_AllCache();
$rows = $msheets->get_CachedSearch($conditions,$limit, $offset,"sheet DESC");
$total = $msheets->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
		array("content" => "#", "class" => "text-center	align-middle"),
		 //array("content" => lang("App.sheet"), "class" => "text-center	align-middle"),
...
$component = '/maintenance/sheets';
$count = $offset;
foreach ($rows["data"] as $row) {
		if(!empty($row["sheet"])){
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
				$hrefView="$component/view/{$row["sheet"]}";
				$hrefEdit="$component/edit/{$row["sheet"]}";
				$hrefDelete="$component/delete/{$row["sheet"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
				...
				$bgrid->add_Row(
						array(
								array("content" => $count, "class" => "text-center align-middle"),
								 //array("content" => $row['sheet'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card("card-grid", array(
		"title" =>lang('Sheets.list-title'),
		"header-back" => $back,
		"header-add"=>"/sie/q10files/import/" . lpk(),
		"alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sheets.list-title'), "message" => lang('Sheets.list-description')),
		"content" => $bgrid,
));
echo($card);
?>
