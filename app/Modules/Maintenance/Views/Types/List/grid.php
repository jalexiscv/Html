...
/**
* █ ---------------------------------------------------------------------------------------------------------------------
* █ ░FRAMEWORK                                  2025-04-08 12:28:06
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Maintenance\Views\Types\List\table.php]
...
**/
//[models]--------------------------------------------------------------------------------------------------------------
$mtypes = model('App\Modules\Maintenance\Models\Maintenance_Types');
//[vars]----------------------------------------------------------------------------------------------------------------
$back= "/maintenance";
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mtypes->clear_AllCache();
$rows = $mtypes->get_CachedSearch($conditions,$limit, $offset,"type DESC");
$total = $mtypes->get_CountAllResults($conditions);
...
$component = '/maintenance/types';
$count = $offset;
foreach ($rows["data"] as $row) {
		if(!empty($row["type"])){
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
				$hrefView="$component/view/{$row["type"]}";
				$hrefEdit="$component/edit/{$row["type"]}";
				$hrefDelete="$component/delete/{$row["type"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
