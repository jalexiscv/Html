...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mcountries, esta deberá ser igualada a  model('App\Modules\Application\Models\Application_Countries');
 * @Instruction $mcountries = model('App\Modules\Settings\Models\Settings_Countries');
 ...
 */
class Settings_Countries extends CachedModel
{
     ...
     * @return array|false
     */
    public function getCountry($country): false|array
    {
        $result = parent::getCached($country);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
