 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Application\Views\Cities\List\table.php]
...
//$mCities->clear_AllCache();
$rows = $mCities->getCachedSearch($conditions, $limit, $offset, "city DESC");
$total = $mCities->getCountAllResults($conditions);
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Settings_Cities.list-title'),
    //"header-back" => $back,
    "header-add" => "/settings/cities/create/{$oid}",
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Settings_Cities.list-title'), "message" => lang('Settings_Cities.list-description')),
    "content" => $bgrid,
));
echo($card);
?>