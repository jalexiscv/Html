...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-05-05 15:16:08
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Application\Views\Countries\Editor\form.php]
 ...
 **/
$row = $model->getCountri($oid);
$r["country"] = $f->get_Value("country", $row["country"]);
$r["name"] = $f->get_Value("name", $row["name"]);
$r["_iso3"] = $f->get_Value("_iso3", $row["_iso3"]);
$r["codigo"] = $f->get_Value("codigo", $row["codigo"]);
...
$f->fields["_iso3"] = $f->get_FieldText("_iso3", array("value" => $r["_iso3"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
$f->fields["codigo"] = $f->get_FieldText("codigo", array("value" => $r["codigo"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
...
//[groups]----------------------------------------------------------------------------------------------------------------
$f->groups["g1"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["country"] . $f->fields["name"] . $f->fields["_iso3"])));
$f->groups["g2"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["codigo"] . $f->fields["status"] . $f->fields["date"])));
$f->groups["g3"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["time"] . $f->fields["author"] . $f->fields["created_at"])));
...
$card = $bootstrap->get_Card("create", array(
    "title" => lang("Settings_Countries.edit-title"),
    "content" => $f,
    "header-back" => $back
));
echo($card);
