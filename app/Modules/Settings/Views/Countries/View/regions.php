...
$rows = $mregions->getCachedSearch($conditions, $limit, $offset, "region ASC");
...
foreach ($rows["data"] as $row) {
    if (!empty($row["region"])) {
        ...
        //[etc]---------------------------------------------------------------------------------------------------------
        $count_cities = $mcities->getCountByRegion($row["region"]);
        $bgrid->add_Row(
            array(
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Settings_Regions.list-title'),
    //"header-back" => $back,
    "header-add" => "/settings/regions/create/{$oid}",
    "alert" => array("icon" => ICON_INFO,
        "type" => "info",
        "title" => lang('Settings_Regions.list-title'),
        "message" => lang('Settings_Regions.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
