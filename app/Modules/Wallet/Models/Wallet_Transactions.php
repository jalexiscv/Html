...
class Wallet_Transactions extends Model
{
    ...
    public function regenerate()
    {
        ...
        $fields = [
            'transaction' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
            ...
            'debit' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
            'credit' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
            'balance' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    {
        $result = $this
            ->select("SUM(`debit`) AS `tdebit`, SUM(`credit`) AS `tcredit`")
            ->where("user", $user)
            ->first();
        if (is_array($result)) {
            $balance = $result["tcredit"] - $result["tdebit"];
            return ($balance);
        } else {
            return (0.0);
        }
