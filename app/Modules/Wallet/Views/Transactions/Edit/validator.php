<?php
$prefix = "Wallet.Transactions";
$f = service("forms", array("lang" => "{$prefix}"));
/** Request **/
//$f->set_ValidationRule("transaction","trim|required");
//$f->set_ValidationRule("module","trim|required");
//$f->set_ValidationRule("user","trim|required");
//$f->set_ValidationRule("reference","trim|required");
//$f->set_ValidationRule("currency","trim|required");
//$f->set_ValidationRule("debit","trim|required");
//$f->set_ValidationRule("credit","trim|required");
//$f->set_ValidationRule("balance","trim|required");
//$f->set_ValidationRule("status","trim|required");
//$f->set_ValidationRule("author","trim|required");
//$f->set_ValidationRule("created_at","trim|required");
//$f->set_ValidationRule("updated_at","trim|required");
//$f->set_ValidationRule("deleted_at","trim|required");
/** Validation **/
$d["module"] = $module;
$d["component"] = $component;
if ($f->run_Validation()) {
    $c = view($namespaced . '\processor', $d);
} else {
    $alert = service("smarty");
    $alert->assign("title", lang("{$prefix}update-validation-title"));
    $alert->assign("message", lang("{$prefix}update-validation-message"));
    $alert->assign("errors", $f->validation->listErrors());
    //$alert->assign("voice","Wallet-Transactions-update-validation-danger-voice.mp3");
    $c = $alert->view('alerts/danger.tpl');
    $c .= view($namespaced . '\form', $d);
}
echo($c);
?>
