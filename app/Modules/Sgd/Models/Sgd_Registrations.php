...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mregistrations, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Registrations');
 * @Instruction $mregistrations = model('App\Modules\Sgd\Models\Sgd_Registrations');
 ...
 */
class Sgd_Registrations extends CachedModel
{
    ...
    {
        $result = $this->get_CachedFirst([$this->primaryKey => $registration]);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
