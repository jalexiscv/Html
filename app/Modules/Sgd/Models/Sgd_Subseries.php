...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $msubseries, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Subseries');
 * @Instruction $msubseries = model('App\Modules\Sgd\Models\Sgd_Subseries');
 ...
 */
class Sgd_Subseries extends CachedModel
{
     ...
     */
    public function get_SelectData($serie)
    {
        $result = $this
    ...
    {
      return($this->getCachedFirst(["subserie" => $subserie]));
    }

    public function getSearch($serie, array $conditions = [], int $limit = 0, int $offset = 0, string $orderBy = '', int $page = 1): array
    {
        $search = @$conditions['search'];
        if (!empty($search)) {
            $this->where("serie", $serie);
            $this->groupStart();
            $this->like("subserie", "%{$search}%");
