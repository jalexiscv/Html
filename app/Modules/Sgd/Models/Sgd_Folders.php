...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mfolders, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Folders');
* @Instruction $mfolders = model('App\Modules\Sgd\Models\Sgd_Folders');
...
*/
class Sgd_Folders extends CachedModel
	 {
		 protected $table = "sgd_folders";
		 protected $primaryKey = "folder";
...
/**
		 ...
		 */
		 public function get_SelectData($box)
			 {
				 $result = $this
		 ...
		 * @return array|false
		 */
		 public function getFolder($folder):false|array
		{
				$result = parent::getCached($folder);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
