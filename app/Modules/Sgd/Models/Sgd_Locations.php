...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mlocations, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Locations');
* @Instruction $mlocations = model('App\Modules\Sgd\Models\Sgd_Locations');
...
*/
class Sgd_Locations extends CachedModel
	 {
...
/**
		 ...
		 * @return array|false
		 */
		 public function getLocation($location):false|array
		{
				$result = parent::getCached($location);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
