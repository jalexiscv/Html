...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mboxes, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Boxes');
* @Instruction $mboxes = model('App\Modules\Sgd\Models\Sgd_Boxes');
...
*/
class Sgd_Boxes extends CachedModel
	 {
		 protected $table = "sgd_boxes";
		 protected $primaryKey = "box";
...
/**
		 ...
		 */
		 public function get_SelectData($shelve)
			 {
				 $result = $this
		 ...
		 * @return array|false
		 */
		 public function getBox($box):false|array
		{
				$result = parent::getCached($box);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
