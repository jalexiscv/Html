...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mversions, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Versions');
 * @Instruction $mversions = model('App\Modules\Sgd\Models\Sgd_Versions');
 ...
 */
class Sgd_Versions extends CachedModel
{
    protected $table = "sgd_versions";
    ...
    {
        $result = parent::readCache($version);
        if (is_array($result)) {
            return ($result);
        } else {
            return($this->getCachedFirst(["version" => $version]));
        }
    }
}
