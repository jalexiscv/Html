...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $munits, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Units');
 * @Instruction $munits = model('App\Modules\Sgd\Models\Sgd_Units');
 ...
 */
class Sgd_Units extends CachedModel
{
    protected $table = "sgd_units";
     ...
     * @return array|false
     */
    public function get_Unit($unit): false|array
    {
        $result=$this->where("unit", $unit)->orderBy("created_at", "DESC")->first();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
    ...
    /**
     * Retorna el numero de series que contiene la version indicada
     * @param $version
     * @return int
     */
    public function getCount($version):int
    {
        $result=$this->where("version",$version)->countAllResults();
        return $result;
    }
