...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mshelves, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Shelves');
 * @Instruction $mshelves = model('App\Modules\Sgd\Models\Sgd_Shelves');
 ...
 */
class Sgd_Shelves extends CachedModel
{
    protected $table = "sgd_shelves";
    protected $primaryKey = "shelve";
     ...
     */
    public function get_SelectData($center)
    {
        $result = $this
     ...
     * @return array|false
     */
    public function getShelve($shelve): false|array
    {
        $result = parent::getCached($shelve);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
