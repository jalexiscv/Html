...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mcenters, esta deberá ser igualada a  model('App\Modules\Sgd\Models\Sgd_Centers');
* @Instruction $mcenters = model('App\Modules\Sgd\Models\Sgd_Centers');
...
*/
class Sgd_Centers extends CachedModel
	 {
		 protected $table = "sgd_centers";
		 protected $primaryKey = "center";
...
/**
		 ...
		 * @return array|false
		 */
		 public function getCenter($center):false|array
		{
				$result = parent::getCached($center);
				if (is_array($result)) {
						return ($result);
				} else {
						return (false);
