 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Versions\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mversions->clear_AllCache();
$rows = $mversions->get_CachedSearch($conditions, $limit, $offset, "version DESC");
$total = $mversions->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("App.Units"), "class" => "text-center	align-middle"),
...
$component = '/sgd/versions';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["version"])) {
        $count++;
        $countunits=$munits->getCount($row["version"]);
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["version"]}";
        $hrefEdit = "$component/edit/{$row["version"]}";
        $hrefDelete = "$component/delete/{$row["version"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
        ...
        $bgrid->add_Row(
            array(
                ...
                array("content" => $countunits, "class" => "text-center align-middle"),
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sgd_Versions.list-title'),
    "header-back" => $back,
    "header-add" => "/sgd/versions/create/" . lpk(),
    "content" => $bgrid,
    "alert" =>array(
        "type" => "info",
        "title" => lang("App.Warning"),
        "message" => lang("Sgd_Versions.list-info"),
    ),
));
echo($card);
?>