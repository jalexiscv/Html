...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Shelves\Delete\form.php]
...
$r= $model->getShelve($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Sgd_Shelves.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Sgd_Shelves.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Sgd_Shelves.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
