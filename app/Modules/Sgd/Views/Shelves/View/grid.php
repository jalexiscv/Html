...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-03-27 04:42:02
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Boxes\List\table.php]
...
//$mboxes->clear_AllCache();
$rows = $mboxes->get_CachedSearch($conditions, $limit, $offset, "box DESC");
$total = $mboxes->get_CountAllResults($conditions);
...
foreach ($rows["data"] as $row) {
    if (!empty($row["box"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["box"]}";
        $hrefEdit = "$component/edit/{$row["box"]}";
        $hrefDelete = "$component/delete/{$row["box"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sgd_Boxes.list-title'),
    "header-add" => "/sgd/boxes/create/{$oid}",
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sgd_Boxes.list-title'), "message" => lang('Sgd_Boxes.list-description')),
    "content" => $bgrid,
));
echo($card);
?>