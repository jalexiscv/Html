...
$fields = array(
		 //"subserie" => lang("App.subserie"),
		 //"serie" => lang("App.serie"),
		 //"name" => lang("App.name"),
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$msubseries->clear_AllCache();
$rows = $msubseries->get_CachedSearch($conditions,$limit, $offset,"subserie DESC");
$total = $msubseries->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
		array("content" => "#", "class" => "text-center	align-middle"),
		 //array("content" => lang("App.subserie"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.serie"), "class" => "text-center	align-middle"),
...
foreach ($rows["data"] as $row) {
		if(!empty($row["subserie"])){
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
				$hrefView="$component/view/{$row["subserie"]}";
				$hrefEdit="$component/edit/{$row["subserie"]}";
				$hrefDelete="$component/delete/{$row["subserie"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
				...
				$bgrid->add_Row(
						array(
								array("content" => $count, "class" => "text-center align-middle"),
								 //array("content" => $row['subserie'], "class" => "text-left align-middle"),
								 //array("content" => $row['serie'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card("card-grid", array(
		"title" =>lang('Sgd_Subseries.list-title'),
		"header-back" => $back,
		"header-add"=>"/sie/q10files/import/" . lpk(),
		"alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sgd_Subseries.list-title'), "message" => lang('Sgd_Subseries.list-description')),
		"content" => $bgrid,
));
echo($card);
?>
