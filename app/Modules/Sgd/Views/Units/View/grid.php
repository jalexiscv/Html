...
$limit = !empty($request->getVar("limit")) ? $request->getVar("limit") :500;
$fields = array(
...
//$mseries->clear_AllCache();
$rows = $mseries->get_CachedSearch($conditions, $limit, $offset, "reference ASC");
$total = $mseries->get_CountAllResults($conditions);
...
foreach ($rows["data"] as $row) {
    if (!empty($row["serie"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["serie"]}";
        $hrefEdit = "$component/edit/{$row["serie"]}";
        $hrefDelete = "$component/delete/{$row["serie"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sgd_Series.list-title'),
    //"header-back" => $back,
    "header-add" => "/sgd/series/create/{$oid}",
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sgd_Series.list-title'), "message" => lang('Sgd_Series.list-description')),
    "content" => $bgrid,
));
echo($card);
?>