...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Folders\Delete\form.php]
...
$r= $model->getFolder($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Sgd_Folders.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Sgd_Folders.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Sgd_Folders.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
