...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Locations\Delete\form.php]
...
$r= $model->get_Locations($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Locations.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Locations.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Locations.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
