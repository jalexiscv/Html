...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-03-27 05:26:14
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Folders\List\table.php]
...
//$mfolders->clear_AllCache();
$rows = $mfolders->get_CachedSearch($conditions, $limit, $offset, "folder DESC");
$total = $mfolders->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => "Carpetas", "class" => "text-center	align-middle"),
...
$component = '/sgd/folders';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["folder"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["folder"]}";
        $hrefEdit = "$component/edit/{$row["folder"]}";
        $hrefDelete = "$component/delete/{$row["folder"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sgd_Folders.list-title'),
    "header-add" => "/sgd/folders/create/{$oid}",
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sgd_Folders.list-title'), "message" => lang('Sgd_Folders.list-description')),
    "content" => $bgrid,
));
echo($card);
?>