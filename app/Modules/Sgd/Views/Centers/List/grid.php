...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-03-27 00:41:04
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Centers\List\table.php]
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mcenters->clear_AllCache();
$rows = $mcenters->get_CachedSearch($conditions, $limit, $offset, "center DESC");
$total = $mcenters->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    array("content" => lang("Sgd_Centers.document-management-centers-code"), "class" => "text-center text-nwrap	align-middle"),
    ...
    array("content" => lang("Sgd_Centers.document-management-centers"), "class" => "text-center	align-middle"),
...
$component = '/sgd/centers';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["center"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["center"]}";
        $hrefEdit = "$component/edit/{$row["center"]}";
        $hrefDelete = "$component/delete/{$row["center"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sgd_Centers.list-title'),
    "header-back" => $back,
    "header-add" => "/sgd/centers/create/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sgd_Centers.list-title'), "message" => lang('Sgd_Centers.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
