...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-03-27 03:27:03
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sgd\Views\Shelves\List\table.php]
...
//$mshelves->clear_AllCache();
$rows = $mshelves->get_CachedSearch($conditions, $limit, $offset, "shelve DESC");
$total = $mshelves->get_CountAllResults($conditions);
...
$component = '/sgd/shelves';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["shelve"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["shelve"]}";
        $hrefEdit = "$component/edit/{$row["shelve"]}";
        $hrefDelete = "$component/delete/{$row["shelve"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card2("card-grid", array(
    "header-title" => lang('Sgd_Shelves.list-title'),
    "header-add" => "/sgd/shelves/create/{$oid}",
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Sgd_Shelves.list-title'), "message" => lang('Sgd_Shelves.list-description')),
    "content" => $bgrid,
));
echo($card);
?>