...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-05-05 18:52:58
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Application\Views\Towns\List\table.php]
 ...
 **/
//[models]--------------------------------------------------------------------------------------------------------------
$mtowns = model('App\Modules\Application\Models\Application_Towns');
//[vars]----------------------------------------------------------------------------------------------------------------
$back = "/application";
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mtowns->clear_AllCache();
$rows = $mtowns->getCachedSearch($conditions, $limit, $offset, "city DESC");
$total = $mtowns->getCountAllResults($conditions);
...
$component = '/application/towns';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["city"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["city"]}";
        $hrefEdit = "$component/edit/{$row["city"]}";
        $hrefDelete = "$component/delete/{$row["city"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card("card-grid", array(
    "title" => lang('Towns.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/q10files/import/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Towns.list-title'), "message" => lang('Towns.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
