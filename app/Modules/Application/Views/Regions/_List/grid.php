...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2025-05-05 16:14:52
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Application\Views\Regions\List\table.php]
 ...
 **/
//[models]--------------------------------------------------------------------------------------------------------------
$mregions = model('App\Modules\Application\Models\Application_Regions');
//[vars]----------------------------------------------------------------------------------------------------------------
$back = "/application";
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mregions->clear_AllCache();
$rows = $mregions->getCachedSearch($conditions, $limit, $offset, "region DESC");
$total = $mregions->getCountAllResults($conditions);
...
$component = '/application/regions';
$count = $offset;
foreach ($rows["data"] as $row) {
    if (!empty($row["region"])) {
        $count++;
        //[links]-------------------------------------------------------------------------------------------------------
        $hrefView = "$component/view/{$row["region"]}";
        $hrefEdit = "$component/edit/{$row["region"]}";
        $hrefDelete = "$component/delete/{$row["region"]}";
        //[buttons]-----------------------------------------------------------------------------------------------------
...
$card = $bootstrap->get_Card("card-grid", array(
    "title" => lang('Regions.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/q10files/import/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Regions.list-title'), "message" => lang('Regions.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
