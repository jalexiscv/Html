...
namespace Modules\Internationalization\Models;
...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mlanguages, esta deberá ser igualada a  model('App\Modules\Application\Models\Application_Languages');
 * @Instruction $mlanguages = model('App\Modules\Application\Models\Application_Languages');
 ...
 */
class Application_Languages extends CachedModel
{
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->like("id", "%{$search}%")
            ->orLike("code", "%{$search}%")
            ->orLike("locale", "%{$search}%")
            ->orLike("name", "%{$search}%")
            ->orLike("is_default", "%{$search}%")
            ->orLike("icon", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Id($id): false|array
    {
        $result = parent::get_Cached($id);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
