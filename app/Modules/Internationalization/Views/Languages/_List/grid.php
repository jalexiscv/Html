...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-12-20 15:54:33
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Application\Views\Languages\List\table.php]
 ...
 **/
//[models]--------------------------------------------------------------------------------------------------------------
$mlanguages = model('Modules\Internationalization\Models\Application_Languages');
//[vars]----------------------------------------------------------------------------------------------------------------
$back = "/application";
...
$fields = array(
    //"id" => lang("App.id"),
    //"code" => lang("App.code"),
    //"locale" => lang("App.locale"),
    //"name" => lang("App.name"),
    //"is_default" => lang("App.is_default"),
    //"icon" => lang("App.icon"),
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mlanguages->clear_AllCache();
$rows = $mlanguages->get_CachedSearch($conditions, $limit, $offset, "id DESC");
$total = $mlanguages->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
    ...
    //array("content" => lang("App.id"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.code"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.locale"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.name"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.is_default"), "class" => "text-center	align-middle"),
    //array("content" => lang("App.icon"), "class" => "text-center	align-middle"),
...
$component = '/application/languages';
$count = $offset;
    ...
    if (!empty($row["id"])) {
        ...
        $bgrid->add_Row(
            array(
                ...
                //array("content" => $row['id'], "class" => "text-left align-middle"),
                //array("content" => $row['code'], "class" => "text-left align-middle"),
                //array("content" => $row['locale'], "class" => "text-left align-middle"),
                //array("content" => $row['name'], "class" => "text-left align-middle"),
                //array("content" => $row['is_default'], "class" => "text-left align-middle"),
                //array("content" => $row['icon'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card("card-grid", array(
    "title" => lang('Languages.list-title'),
    "header-back" => $back,
    "header-add" => "/sie/q10files/import/" . lpk(),
    "alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Languages.list-title'), "message" => lang('Languages.list-description')),
    "content" => $bgrid,
));
echo($card);
?>
