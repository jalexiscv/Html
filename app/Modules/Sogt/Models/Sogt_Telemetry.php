...
/**
 * @Copilot Siempre que en el código se cree una variable llamada $mtelemetry, esta deberá ser igualada a  model('App\Modules\Sogt\Models\Sogt_Telemetry');
 * @Instruction $mtelemetry = model('App\Modules\Sogt\Models\Sogt_Telemetry');
 ...
 */
class Sogt_Telemetry extends CachedModel
{
    protected $table = "sogt_telemetry";
    protected $primaryKey = "telemetry";
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("telemetry", "%{$search}%")
            ->orLike("device", "%{$search}%")
            ->orLike("user", "%{$search}%")
            ->orLike("latitude", "%{$search}%")
            ->orLike("longitude", "%{$search}%")
            ->orLike("altitude", "%{$search}%")
            ->orLike("speed", "%{$search}%")
            ->orLike("heading", "%{$search}%")
            ->orLike("gps_valid", "%{$search}%")
            ->orLike("satellites", "%{$search}%")
            ->orLike("network", "%{$search}%")
            ->orLike("battery", "%{$search}%")
            ->orLike("ignition", "%{$search}%")
            ->orLike("event", "%{$search}%")
            ->orLike("motion", "%{$search}%")
            ->orLike("timestamp", "%{$search}%")
     ...
     * @return array|false
     */
    public function getTelemetry($telemetry): false|array
    {
        $result = parent::getCached($telemetry);
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
