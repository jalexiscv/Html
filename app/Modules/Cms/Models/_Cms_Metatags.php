...
class Cms_Metatags extends Model
{
     ...
     * @return array|false
     */
    public function get_Meta($meta): false|array
    {
        $key = $this->get_CacheKey("meta-{$meta}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $meta)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
