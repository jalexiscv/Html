...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-08-24 09:43:33
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Cms\Views\Menus\Editor\form.php]
...
//[vars]----------------------------------------------------------------------------------------------------------------
$row = $model->get_Menus($oid);
$r["menu"] = $f->get_Value("menu", $row["menu"]);
$r["post"] = $f->get_Value("post", $row["post"]);
$r["men_menu"] = $f->get_Value("men_menu", $row["men_menu"]);
...
$back = "/cms/menus/list/" . lpk();
//[fields]----------------------------------------------------------------------------------------------------------------
$f->fields["menu"] = $f->get_FieldText("menu", array("value" => $r["menu"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
$f->fields["post"] = $f->get_FieldText("post", array("value" => $r["post"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
$f->fields["men_menu"] = $f->get_FieldText("men_menu", array("value" => $r["men_menu"], "proportion" => "col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12"));
...
//[groups]----------------------------------------------------------------------------------------------------------------
$f->groups["g1"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["menu"] . $f->fields["post"] . $f->fields["men_menu"])));
$f->groups["g2"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["title"] . $f->fields["description"] . $f->fields["parent"])));
$f->groups["g3"] = $f->get_Group(array("legend" => "", "fields" => ($f->fields["target"] . $f->fields["author"] . $f->fields["created_at"])));
