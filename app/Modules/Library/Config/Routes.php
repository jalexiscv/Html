...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Library\Models\Library_Modules");
    $mdcxm = model("App\Modules\Library\Models\Library_Clients_Modules");
    $module = 'library';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Library::index');
                $subroutes->add('/home', 'Library::index');
                $subroutes->add('home/(:any)', 'Library::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Library::denied');
                $subroutes->add('(:any)', 'Library::denied');
            }
        );
    }
}
