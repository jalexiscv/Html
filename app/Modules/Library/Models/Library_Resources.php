 ...
 */
class Library_Resources extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'resource' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'title' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'description' => ['type' => 'TEXT', 'null' => FALSE],
                'authors' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'use' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'category' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'level' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'objective' => ['type' => 'TEXT', 'null' => FALSE],
                'program' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'type' => ['type' => 'VARCHAR', 'constraint' => 32, 'null' => FALSE],
                'format' => ['type' => 'VARCHAR', 'constraint' => 32, 'null' => FALSE],
                'language' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'file' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'url' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'keywords' => ['type' => 'TINYTEXT', 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("level", "%{$search}%")
            ->orLike("objective", "%{$search}%")
            ->orLike("program", "%{$search}%")
            ->orLike("type", "%{$search}%")
            ->orLike("format", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Resource($resource): false|array
    {
        $key = $this->get_CacheKey("resource-{$resource}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $resource)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
