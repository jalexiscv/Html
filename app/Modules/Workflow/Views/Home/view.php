...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                                                    2025-05-15 12:29:19
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Workflow\Home\breadcrumb.php]
...
/** @var object $request */
generate_workflow_permissions($module);
$bootstrap = service("bootstrap");
$server = service("server");
$version = round(($server->get_DirectorySize(APPPATH . 'Modules/Workflow') / 102400), 6);
$card = $bootstrap->get_Card("card-view-Workflow", array(
    "class" => "mb-3",
    "title" => lang("Workflow.module") . "<span class='text-muted'>v{$version}</span>",
    "header-back" => "/",
    "image" => "/themes/assets/images/header/module-workflow.png",
    "image-class" => "img-fluid p-3",
    "content" => lang("Workflow.intro-1")
));
echo($card);

if ($authentication->get_LoggedIn() && $authentication->has_Permission("workflow-access")) {
