...
if ($platform->get_CandidateModule(__FILE__)) {
    ...
    $routes->group($module, ['namespace' => $namespace], function ($subroutes) use ($authorized) {
        if ($authorized === 'authorized') {
            $subroutes->add('', 'Workflow::index');
            $subroutes->add('home', 'Workflow::index');
            $subroutes->add('home/(:any)', 'Workflow::home/$1');
            $subroutes->add('(:any)/(:any)/(:any)', 'Router::route/$1/$2/$3');
        } else {
            $subroutes->add('/', 'Workflow::denied');
            $subroutes->add('(:any)', 'Workflow::denied');
        }
    });
}
?>
