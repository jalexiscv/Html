...
class AttributeSpec extends ObjectBehavior
{
    public function it_can_alter_the_values()
    {
        ...
        $this->alter(static function ($values) {
            return array_map('strtoupper', $values);
        })->render()->shouldReturn('name="HELLO WORLD"');
        $this->alter(static function (array $values) {
            $values[] = 'foo';
            $values[] = '0';
            $values[] = [new stdClass(),];
            $values[] = new randomPrintableObject('<a href="#htmltag">htmltag</a>');
            return $values;
        })->render()->shouldReturn('name="HELLO WORLD foo 0 &lt;a href=&quot;#htmltag&quot;&gt;htmltag&lt;/a&gt;"');
    }

    public function it_can_append()
    {
        $this->beConstructedWith('name');
        $this->append('a')->shouldReturn($this);
    ...
    public function it_can_be_serialized()
    {
        $this->beConstructedWith('class');
        $this->append('hello world');
        $this->serialize()->shouldReturn('a:2:{s:4:"name";s:5:"class";s:6:"values";a:1:{i:0;s:11:"hello world";}}');
    }
    ...
    public function it_can_be_unserialized()
    {
        $this->beConstructedWith('class');
        $this->append('hello world');
        $this->unserialize('a:2:{s:4:"name";s:5:"class";s:6:"values";a:1:{i:0;s:11:"hello world";}}');
    ...
    public function it_can_be_used_like_an_array()
    {
        ...
        $this->shouldThrow('\BadMethodCallException')->during('offsetGet', ['random']);
        unset($this['world']);
    ...
    public function it_can_render_value_as_string_or_array()
    {
        ...
        $this->getValuesAsString()->shouldReturn('0 hello foo bar &quot;&lt;a href=&quot;#&quot;&gt;oops&lt;/a&gt;');
        $this->getValuesAsArray()->shouldReturn(['0', 'hello', 'foo', 'bar', '"<a href="#">oops</a>',]);
    }

    public function it_can_replace_values()
    {
        $this->beConstructedWith('name', 'c b a');
        $this->replace('a', 'A')->shouldReturn($this);
        $this->getValuesAsArray()->shouldBe(['c b a']);
        $this->render()->shouldBe('name="c b a"');
    ...
    {
        $this->shouldThrow('\InvalidArgumentException')->during('__construct', [' name ', 'plop']);
        $this->shouldThrow('\InvalidArgumentException')->during('__construct', ['na me', 'plop']);
        $this->shouldThrow('\InvalidArgumentException')->during('__construct', ['na"me', 'plop']);
        $this->beConstructedWith('name', 'this is a"><script>alert("XSS")</script>');
        $this->getName()->shouldBe('name');
        $this->getValuesAsArray()->shouldBe(['this is a"><script>alert("XSS")</script>']);
        $this->render()->shouldBe('name="this is a&quot;&gt;&lt;script&gt;alert(&quot;XSS&quot;)&lt;/script&gt;"');
    }

    public function it_can_set()
    {
        $this->beConstructedWith('name', 'value');
        $this->set('anything')->shouldReturn($this);
        $this->render()->shouldReturn('name="anything"');
    }
