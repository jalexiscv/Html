...
class TagSpec extends ObjectBehavior
{
    public function it_can_alter_the_content()
    {
        ...
        })->render()->shouldReturn('<p/>');
        $this->content('hello content', 'goodbye content');
        $this->alter(static function (array $values) {
            return array_map('strtoupper', $values);
        })->render()->shouldReturn('<p>HELLO CONTENTGOODBYE CONTENT</p>');
        $this->alter(static function (array $values) {
            $values[] = '<a href="#htmltag">htmltag</a>';
            $values[] = new stdClass();
            $values[] = ['boo', 'foo', 'bar', [new randomPrintableObject('<a href="#htmltag">htmltag</a>'),],];
            return $values;
        })->render()->shouldReturn('<p>HELLO CONTENTGOODBYE CONTENT&lt;a href=&quot;#htmltag&quot;&gt;htmltag&lt;/a&gt;boofoobar<a href="#htmltag">htmltag</a></p>');
    }
    ...
    public function it_should_create_a_tag()
    {
        $attributes = AttributesFactory::build();
        $this->beConstructedWith($attributes, 'p');
        $this->attr('class')->append('paragraph');
        $this->render()->shouldBe('<p class="paragraph"/>');
        $subtag = new Tag(AttributesFactory::build(), 'b');
        $subtag->content(['bold text']);
        $this->content(['hello ', $subtag, $subtag->render()]);
        $this->render()->shouldBe('<p class="paragraph">hello <b>bold text</b>&lt;b&gt;bold text&lt;/b&gt;</p>');
    }
