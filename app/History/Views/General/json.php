...
* -----------------------------------------------------------------------------
*  ╔═╗╔╗╔╔═╗╔═╗╦╔╗ ╦  ╔═╗
*  ╠═╣║║║╚═╗╚═╗║╠╩╗║  ║╣  [FRAMEWORK][App\Modules\Application\Views\Stats\List\json.php]
*  ╩ ╩╝╚╝╚═╝╚═╝╩╚═╝╩═╝╚═╝
* -----------------------------------------------------------------------------
...
$list = $mstats
    ->where("instance", $authentication->get_Client())
    ->groupStart()
    ->like("stat", "%{$search}%")
    ->orLike("user", "%{$search}%")
    ->groupEnd()
...
foreach ($list as $item) {
    $alias = $strings->get_Strtolower($muserfields->get_AliasByUser($item["user"]));
    $avatar = "/themes/bs5/img/avatars/avatar-neutral.png";
    $attachment = $mattachments->where("object", $item["user"])->where("reference", "AVATAR")->orderBy("created_at", "DESC")->first();
    ...
    */
    ...
    $options = $bootstrap::get_BtnGroup('options', array($lviewer));
    $row["stat"] = $item["stat"];
    ...
    $row["ip"] = $item["ip"];
    $row["user"] = $item["user"];
    $row["avatar"] = "<img class=\"avatar\" src=\"{$avatar}\" width=\"36\">";
    $row["alias"] = $alias;
    $row["type"] = $item["type"];
