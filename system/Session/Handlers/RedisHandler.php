...
class RedisHandler extends BaseHandler
{
    ...
    public function __construct(AppConfig $config, string $ipAddress)
    {
        ...
        if ($session instanceof SessionConfig) {
            $this->sessionExpiration = empty($session->expiration) ? (int)ini_get('session.gc_maxlifetime') : (int)$session->expiration;
    ...
    protected function setSavePath(): void
    {
        ...
        if (preg_match('#(?:tcp://)?([^:?]+)(?:\:(\d+))?(\?.+)?#', $this->savePath, $matches)) {
            ...
            $this->savePath = ['host' => $matches[1], 'port' => empty($matches[2]) ? self::DEFAULT_PORT : $matches[2], 'password' => preg_match('#auth=([^\s&]+)#', $matches[3], $match) ? $match[1] : null, 'database' => preg_match('#database=(\d+)#', $matches[3], $match) ? (int)$match[1] : 0, 'timeout' => preg_match('#timeout=(\d+\.\d+|\d+)#', $matches[3], $match) ? (float)$match[1] : 0.0,];
